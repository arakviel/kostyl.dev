...
Компонент для рендерингу математичних формул за допомогою бібліотеки KaTeX. Підтримує блоковий та вбудований (inline) режими.

**Синтаксис (через слот):**
...
**Синтаксис (через пропс):**

````markdown
::math-formula{tex="E = mc^2" :block="false"}
::
````

Або коротший варіант (вбудований inline):

````markdown
Це формула :math-formula{tex="E = mc^2" inline} у тексті.
````
...
| Атрибут | Тип     | За замовчуванням | Опис                                                                        |
| ------- | ------- | ---------------- | --------------------------------------------------------------------------- |
| `tex`   | string  | `""`             | Формула у форматі LaTeX. Має нижчий пріоритет, якщо передано вміст у слот. |
| `block` | boolean | `true`           | Рендерити як окремий блок по центру (display mode) чи inline (`false`).    |
| `inline`| boolean | `false`          | Вмикає вбудований (inline) режим. Має вищий пріоритет над `block`.          |
