...
description: 'Розуміння структури програм на C# є фундаментальним для ефективної розробки. Ця тема охоплює еволюцію від класичного підходу до сучасних [top-level statements](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/program-structure/top-level-statements), а також механізми документування коду.'
---
...
Для виведення одного рядка потрібно було визначити [namespace](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/namespaces), клас та метод [Main](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/program-structure/main-command-line). Це створювало бар'єр для початківців та ускладнювало написання простих скриптів.
...
#### 1. [Using Directives (Директиви імпорту)](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/using-directive)
...
Метод `Main` — це **спеціальний метод**, який виконує [CLR (Common Language Runtime)](https://learn.microsoft.com/en-us/dotnet/standard/clr) при запуску програми.
...
```csharp [Program.cs]
// Top-level statements - без namespace, class, Main!
Console.WriteLine("Hello, World!");
```
...
Магічна змінна [`args`](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/program-structure/main-command-line#command-line-arguments) доступна автоматично:
...
## [Implicit Usings (Неявні імпорти)](https://learn.microsoft.com/en-us/dotnet/core/project-sdk/overview#implicit-using-directives)
...
Ви можете додати власні [global using](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/using-directive#global-modifier) директиви, які будуть доступні у всіх файлах:
...
```csharp [GlobalUsings.cs]
// Глобальні using директиви для всього проєкту
global using System.Text.Json;
...
```csharp [Program.cs]
// Завдяки GlobalUsings.cs, JsonSerializer, Regex та Math доступні без using!
var data = new { Name = "C#", Version = 12 };
var json = JsonSerializer.Serialize(data);
Console.WriteLine(json);

var pattern = new Regex(@"\d+");
var match = pattern.Match("C# 12");
...
```csharp [Utilities/StringHelper.cs]
namespace MyApp.Utilities;

public class StringHelper
{
    // JsonSerializer доступний завдяки global using
...
### [XML-документація](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/xmldoc/): Професійний стандарт
...
```csharp [GlobalUsings.cs]
// Централізовані глобальні using директиви
global using System.Text.Json;
global using MyConsoleApp.Models;
global using MyConsoleApp.Services;
```

```csharp [Program.cs]
// Top-level statements - точка входу програми
Console.WriteLine("=== My Console Application ===");

var userService = new UserService();
userService.CreateUser("Олександр", "alex@example.com");
...
public class UserService
{
    ...
    public void CreateUser(string name, string email)
    {
        ...
        _users.Add(user);
        Console.WriteLine($"✓ User '{name}' created with ID {user.Id}");
    }
...
Сучасний консольний застосунок на C# 12 (.NET 8).
...
```
