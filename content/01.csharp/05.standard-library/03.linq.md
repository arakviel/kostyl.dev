...
```csharp {1,2,9-14} showLineNumbers
List<Order> allOrders = GetAllOrders();
...
```csharp [Query Syntax] {1-8} showLineNumbers
...
public static I Enumerable<T> Where<T>(
...
sequenceDiagram
    ...
    style Query fill:#f59e0b,stroke:#b45309,color:#ffffff
    style Iterator fill:#8b5cf6,stroke:#6d28d9,color:#ffffff
```
...
::collapsible{title="InvalidOperationException: Sequence contains no elements"}
...
::collapsible{title="InvalidOperationException: Sequence contains more than one element"}
...
::collapsible{title="NullReferenceException в projection"}
...
::collapsible{title="Collection was modified; enumeration operation may not execute"}
