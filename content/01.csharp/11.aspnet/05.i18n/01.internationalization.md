...
<root>
  <xsd:schema xmlns:xsd="http://www.w3.org/2001/XMLSchema">
    <!-- Стандартна схема .resx — не змінювати -->
  </xsd:schema>
...
namespace ShopApi.Localization;
...
public static class ProductEndpoints
{
    ...
    {
        var (product, errors) = service.Create(request);
...
public static class LocalizationEndpoints
{
    ...
    private static IResult GetSupportedCultures(RequestLocalizationOptions options)
    {
        return Results.Ok(new
        {
...
var localizationOptions = new RequestLocalizationOptions
{
    ...
    SupportedCultures    = supportedCultures,
    SupportedUICultures  = supportedCultures,
    // За замовчуванням провайдери: QueryString, Cookie, AcceptLanguage
    // Тут можна додати власний або змінити порядок
};
...
// ─── Middleware ─────────────────────────────────────────────────
// ВАЖЛИВО: UseRequestLocalization має бути до маппінгу ендпоінтів
app.UseRequestLocalization(localizationOptions);
...
### Товари — українська через Query String
GET http://localhost:5000/products?culture=uk-UA

###

### Товари — польська через Query String
GET http://localhost:5000/products?culture=pl-PL

###

### Локалізована інформація — українська
GET http://localhost:5000/localization/info?culture=uk-UA

###

### Локалізована інформація — польська
GET http://localhost:5000/localization/info?culture=pl

###

### Ціна з локалізованим форматуванням — українська
GET http://localhost:5000/products/1/price?culture=uk-UA

###

### Ціна з локалізованим форматуванням — американська
GET http://localhost:5000/products/1/price?culture=en-US

###

### Помилка — товар не знайдено (українська)
GET http://localhost:5000/products/999?culture=uk-UA

###

### Помилка — товар не знайдено (польська)
GET http://localhost:5000/products/999?culture=pl
...
### Спроба створити порожній товар — валідаційна помилка (українська)
POST http://localhost:5000/products?culture=uk-UA
Content-Type: application/json
