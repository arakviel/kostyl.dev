...
```
dotnet test
├── PostgreSQL Container started (port 5432 mapped to random port)
├── Schema created (migrations)
├── Tests run → queries go to REAL PostgreSQL
...
```bash
# Вимкнути Ryuk (cleanup agent) для прискорення локальних запусків
# (У CI завжди лишайте Ryuk увімкненим!)
