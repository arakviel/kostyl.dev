...
## MVC vs. Minimal API: Що і коли обирати?

Оскільки ви вже знайомі з ASP.NET Core Minimal API, у вас може виникнути логічне запитання: навіщо нам MVC, якщо Minimal API такі швидкі і лаконічні?
...
1. **Організація коду:** У Minimal API розробник сам відповідає за структуру файлів (розбиття на розширення методів, Carter-модулі тощо). MVC надає **строгу конвенцію** з коробки (папки `Controllers/`, `Views/`, `Models/`), що полегшує навігацію у командній розробці.
2. **Рендеринг HTML:** Minimal API створені для повернення даних (JSON/XML). MVC відпочатку проєктувався для повернення HTML-сторінок через потужний механізм шаблонізації Razor. Якщо ви робите серверний рендеринг (Server-Side Rendering) — MVC (або Razor Pages) є безальтернативним вибором.
3. **Фільтри і вбудовані політики:** MVC має зрілу екосистему Action Filters (які ми розглянемо у статті 07) для глобальної обробки помилок, логування чи авторизації на рівні контролерів, що іноді зручніше за Middleware.

Коротке правило: **API для інших програм → Minimal API. Надійний моноліт з HTML-інтерфейсом (або великим REST API з жорсткою структурою) → MVC.**
