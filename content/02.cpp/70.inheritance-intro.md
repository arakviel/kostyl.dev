...
::card{title="✅ Коректне «is-a»" icon="i-lucide-check"}
...
::card{title="❌ Помилкове «is-a»" icon="i-lucide-x"}
...
::plant-uml{alt="Діаграма успадкування Person, Student, Teacher"}
...
package "Об'єкт типу Student" #F8FAFC {
  package "Під'об'єкт Person (базовий клас)" #EFF6FF {
    class "Person" as P {
      + name: "Alice"
      + age: 20
  ...
  package "Поля Student (похідний клас)" #ECFDF5 {
    class "Student" as S {
      + studentId: 12345
...
::debugger-view{title="Local Variables" :variables='[{"name": "alice", "type": "Student", "value": "{...}"}, {"name": "  name", "type": "std::string", "value": "Alice"}, {"name": "  age", "type": "int", "value": "20"}, {"name": "  email", "type": "std::string", "value": "alice@example.com"}, {"name": "  studentId", "type": "int", "value": "12345"}, {"name": "  major", "type": "std::string", "value": "Computer Science"}]'}
::
...
::plant-uml{alt="Діаграма взаємозв'язків класів Person, Student, Teacher, Employee"}
...
::plant-uml{alt="Фізична структура об'єкта Supervisor"}
...
package "Об'єкт типу Supervisor" #F8FAFC {
  package "Під'об'єкт Person" #EFF6FF {
    class "Person" as P {
      + name: "Bob"
      + age: 35
  ...
  package "Під'об'єкт Employee" #FEF3C7 {
    class "Employee" as E {
      + employeeId: 987654
      + wage: 5000
    }
  }
  package "Поля Supervisor" #ECFDF5 {
...
::plant-uml{alt="Класи Dog, Cat, Horse до узагальнення"}
...
::plant-uml{alt="Ієрархія Animal після узагальнення"}
...
::card{title="Узагальнення (Generalization)" icon="i-lucide-arrow-up"}
...
::card{title="Спеціалізація (Specialization)" icon="i-lucide-arrow-down"}
...
::plant-uml{alt="Ієрархія класів Shape, Circle, Rectangle, Square"}
...
:::accordion-item{label="📋 Завдання: Розробка ієрархії транспортних засобів" icon="i-lucide-clipboard-list"}
...
::card{title="Ключові терміни" icon="i-lucide-book-open"}
