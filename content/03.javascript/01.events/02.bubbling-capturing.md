...
<style>
  ...
  .form { background: #dbeafe; border-color: #3b82f6; }
  .div { background: #fef3c7; border-color: #f59e0b; }
  .p { background: #fecaca; border-color: #ef4444; }
</style>
...
<div id="outer" onclick="alert('Не побачите цього')">
  ЗОВНІШНІЙ DIV
  <button id="inner" onclick="event.stopPropagation()">
    Натисни мене — подія зупиниться тут
  </button>
</div>
```
...
:::tab{label="✅ Краще: event.defaultPrevented"}
```javascript
...
:::tab{label="✅ Найкраще: data-атрибути"}
```javascript
...
restrictedArea.addEventListener('click', (e) => {
...
::card
  ---
  title: "🚫 stopPropagation на capturing"
