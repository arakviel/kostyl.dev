...
Можливо, ви запитуєте себе: *"Навіщо мені вчити `XMLHttpRequest` у 2025 році, коли є `fetch`?"*
...
2.  **Upload Progress:** XHR має вбудовану підтримку відстеження прогресу завантаження файлів *на сервер*. У `fetch` це досі реалізується складно (через Streams), а в XHR це працює "з коробки" вже 15 років.
...
> [!NOTE]
> Цей матеріал — це не просто "як зробити запит". Це енциклопедія по роботі з мережею в браузері на низькому рівні. Ми заглибимося в деталі, які зроблять вас *інженером*, а не просто *кодером*.
...
**Важливо:** викликати *після* `open()`, але *перед* `send()`.
...
Це те, заради чого ми тут. XHR має **окремий** механізм для відстеження завантаження даних *на сервер*.
...
> [!important]
> **xhr.onprogress vs xhr.upload.onprogress**
>
...
:::card{icon="lucide:arrow-right-left"}
#title
Sync Support
