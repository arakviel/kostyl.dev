...
participant AccA as Рахунок А
participant AccB as Рахунок Б

    User->>DB: BEGIN TRANSACTION
    DB->>Acc A: UPDATE: -1000₴
    AccA-->>DB: ✅ OK

    critical Що якщо тут crash?
        DB->>DB: 💥 Система впала!
        DB->>DB: ROLLBACK автоматично
        DB->>AccA: Повернути 1000₴ назад
    end

    DB->>AccB: UPDATE: +1000₴
    AccB-->>DB: ✅ OK
    DB->>User: COMMIT - всі зміни збережено
