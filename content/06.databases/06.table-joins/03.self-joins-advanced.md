---
title: SELF JOIN та складні сценарії JOIN
description: Самооб'єднання таблиць, ієрархічні структури, продуктивність JOIN операцій та best practices для реальних проektів
---
...
```mermaid
graph TD
    CEO[Олена Іваненко<br/>CEO<br/>Id:1] --> CTO[Петро Коваль<br/>CTO<br/>Id:2]
    CTO --> TL[Марія Сидорова<br/>Team Lead<br/>Id:3]
    TL --> SD[Іван Петренко<br/>Senior Dev<br/>Id:4]
    TL --> JD[Олег Смирнов<br/>Junior Dev<br/>Id:5]
...
rectangle "Employees (E)" as E EMPLOYEE1 {
    (Id=2, Петро Коваль, ManagerId=1) as E2
    (Id=3, Марія, ManagerId=2) as E3
    (Id=4, Іван, ManagerId=3) as E4
}

rectangle "Employees (M)" as M EMPLOYEE2 {
    (Id=1, Олена CEO) as M1
    (Id=2, Петро CTO) as M2
    (Id=3, Марія TL) as M3
}

E2 -right-> M1 : ManagerId=1 → Id=1
E3 -right-> M2 : ManagerId=2 → Id=2
E4 -right-> M3 : ManagerId=3 → Id=3

note right of M
  Та сама таблиця,
  але розглядається як
  "список менеджерів"
end note
