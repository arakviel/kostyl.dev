...
::tip
**Best Practice:** Використовуйте **Options pattern** для strongly-typed конфігурації замість прямого читання через `IConfiguration["Key"]`. Це дає IntelliSense, compile-time перевірку та легше тестування.
::
...
Це друга частина статті. Я додав:

1. **ARG vs ENV** — детальне порівняння build-time vs runtime змінних
2. **Конфігурація .NET** — як ASP.NET Core читає змінні оточення, синтаксис `__`, Options pattern
3. **Docker Secrets** — проблема секретів у змінних, рішення через файли

Продовжити з фінальними розділами?
- Практичний приклад: Конфігурування C# API для різних середовищ
- Best Practices
