...
### Крок 2: Bucket Policy для публічного читання
...
### Крок 3: Налаштування Static Website Hosting
...
### Крок 5: Збірка та деплой React застосунку

На вашому локальному комп'ютері:

```bash
# Перейдіть у директорію вашого React проєкту
# (або створіть новий: npx create-react-app my-app)
cd my-react-app

# Зберіть production build
# npm — Node Package Manager, аналог NuGet для JavaScript
npm run build
# Команда створить папку build/ з мінімізованим HTML/CSS/JS
```

::terminal-preview{title="npm run build"}

<div class="line"><span class="opacity-40">$</span> <strong>npm run build</strong></div>
<div class="line">Creating an optimized production build...</div>
<div class="line">Compiled successfully.</div>
<div class="line"></div>
<div class="line">File sizes after gzip:</div>
<div class="line">  <span class="text-green-400">47.23 kB  build/static/js/main.abc123.js</span></div>
<div class="line">  1.78 kB   build/static/css/main.def456.css</div>
<div class="line"></div>
<div class="line">The build folder is ready to be deployed.</div>

::

```bash
# Завантажте build/ папку у S3
# aws s3 sync — синхронізація директорії з S3 (завантажує лише змінені файли)
# --delete — видаляє файли з S3, яких немає локально (прибирає старий build)
# ЗАМІНІТЬ my-react-app-2024 на ваш bucket
aws s3 sync ./build s3://my-react-app-2024/ \
    --delete \
    --region eu-central-1
...
<div class="line"><span class="opacity-40">$</span> <strong>aws s3 sync ./build s3://my-react-app-2024/ --delete --region eu-central-1</strong></div>
...
### Крок 6: Lifecycle Policy для старих версій
