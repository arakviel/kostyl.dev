...
*   **Приклад 1: Масове читання профілів користувачів**
    ...
    *   *Крок 1 (Округлення розміру):* $\lceil 3.5\text{ KB} / 4\text{ KB} \rceil = 1$ блок.
    *   *Крок 2 (Розрахунок за формулою):* $1 \times 150 \times 0.5 = 75$ RCU.
*   **Приклад 2: Читання фінансових транзакцій з високою узгодженістю**
    ...
    *   *Крок 1 (Округлення розміру):* $\lceil 9.2\text{ KB} / 4\text{ KB} \rceil = \lceil 2.3\text{ block} \rceil = 3$ блоки.
    *   *Крок 2 (Розрахунок за формулою):* $3 \times 80 \times 1.0 = 240$ RCU.
*   **Приклад 3: Транзакційна перевірка балансу**
    ...
    *   *Крок 1 (Округлення розміру):* $\lceil 1.5\text{ KB} / 4\text{ KB} \rceil = 1$ блок.
    *   *Крок 2 (Розрахунок за формулою):* $1 \times 50 \times 2.0 = 100$ RCU.
...
*   **Приклад 1: Логування IoT-метрик**
    ...
    *   *Крок 1 (Округлення розміру):* $\lceil 0.6\text{ KB} / 1\text{ KB} \rceil = 1$ блок.
    *   *Крок 2 (Розрахунок за формулою):* $1 \times 200 \times 1.0 = 200$ WCU.
*   **Приклад 2: Оновлення профілів користувачів**
    ...
    *   *Крок 1 (Округлення розміру):* $\lceil 2.4\text{ KB} / 1\text{ KB} \rceil = 3$ блоки.
    *   *Крок 2 (Розрахунок за формулою):* $3 \times 45 \times 1.0 = 135$ WCU.
*   **Приклад 3: Оформлення замовлення транзакцією**
    ...
    *   *Крок 1 (Замовлення):* Розмір 1.2 KB округляється до 2 KB. Вартість: $2 \times 10 \times 2.0 = 40$ WCU.
    *   *Крок 2 (Склад):* Розмір 0.8 KB округляється до 1 KB. Вартість: $1 \times 10 \times 2.0 = 20$ WCU.
...
1.  **Витрати на запис (Write Cost Dynamics):**
    ...
    *   Якщо записується новий елемент, споживається $1\text{ WCU}$ (або $2\text{ WCU}$ для транзакцій) за кожен 1 KB розміру елемента як для основної таблиці, так і для кожного LSI, куди проектується цей елемент.
...
| **Обмеження розміру** | Ліміт у $10\ 	ext{GB}$ на колекцію елементів (Item Collection) | Без лімітів (горизонтальне масштабування) |
...
- **1 RCU:** $\approx 0.00013\ \text{USD}$ за годину (що становить приблизно $0.095\ \text{USD}$ на місяць).
- **1 WCU:** $\approx 0.00065\ \text{USD}$ за годину (що становить приблизно $0.474\ \text{USD}$ на місяць).

При резервуванні $100\ \text{WCU}$ та $200\ \text{RCU}$ щомісячна вартість утримання таблиці (без врахування вартості збереження даних) становитиме:
...
- **Обмеження об'єму кошика:** Максимальний об'єм накопичених токенів обмежений часовим інтервалом у **5 хвилин** (300 секунд). Формально, максимальний запас токенів $T_{\max}$ дорівнює:
$$T_{\max} = 300 \times C$$
...
- **1 мільйон RRU:** $\approx 0.284\ \text{USD}$
- **1 мільйон RWU:** $\approx 1.42\ \text{USD}$
...
1. **Історичний пік (Peak Capacity):** DynamoDB гарантує миттєве обслуговування трафіку, який не перевищує подвоєне значення максимального навантаження за останні 30 хвилин. Наприклад, якщо таблиця успішно обробила пік у $10\ 000\ \text{RPS}$, вона може безпосередньо з цього моменту обробити до $20\ 000\ \text{RPS}$.
2. **Поділ партицій при перевищенні піку:** Якщо навантаження перевищує $2 \times \text{Peak}$, DynamoDB запускає фоновий процес поділу партицій (Partition Splitting) для горизонтального розширення сховища та обчислювальних ресурсів. Під час цього процесу (який може тривати до кількох хвилин) запити, що виходять за межі подвоєного піку, можуть зазнавати тимчасового throttling.
3. **Стартові ліміти:** Для абсолютно нової таблиці в режимі On-Demand встановлено ліміти у $4000\ \text{Write Requests/sec}$ та $12\ 000\ \text{Read Requests/sec}$. Ці обмеження автоматично піднімаються в міру зростання реального трафіку.
...
2. Встановити вручну високі показники ємності (наприклад, $10\ 000\ \text{WCU}$ та $20\ 000\ \text{RCU}$), що змусить DynamoDB миттєво виділити необхідну кількість фізичних партицій ("прогріти таблицю").
...
> Транзакційні операції споживають **вдвічі більше** RCU та WCU порівняно зі стандартними запитами. Наприклад, транзакційний запис об'єкта розміром $1\ 	ext{KB}$ споживає $2\ 	ext{WCU}$ (замість $1\ 	ext{WCU}$), а транзакційне strongly consistent читання блоку $4\ 	ext{KB}$ споживає $2\ 	ext{RCU}$ (замість $1\ 	ext{RCU}$).
