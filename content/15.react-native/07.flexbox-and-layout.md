...
![Column default vs web row](/images/react-native/flexbox-and-layout/01.svg){.diagram-img}
...
| `gap` | `gap` (сучасний RN) | Зручніше за «margin на кожному вкладеному елементі» |
...
![Main axis і cross axis](/images/react-native/flexbox-and-layout/02.svg){.diagram-img}
...
- **Cross axis** — **перпендикулярна** до main: якщо main — зверху вниз, cross — ліворуч/праворуч; якщо main — зліва направо, cross — зверху вниз.

Властивість **`flexDirection`** обирає main axis:

| Значення | Main axis | Вкладені елементи… |
| -------- | ------------ | ----------- |
...
::field{name="justifyContent" type="уздовж main axis"}
...
::field{name="alignItems" type="уздовж cross axis"}
...
::field{name="alignSelf" type="на одному елементі"}
  ...
  return (
    <View style={[styles.screen, isDark && { backgroundColor: '#000' }]}>
      ...
      <Text style={[styles.hint, { color: muted }]}>
        У RN без flexDirection вкладені елементи йдуть колонкою — як у першій панелі.
      </Text>
    </View>
  );
}
...
export default function App() {
  ...
  return (
    <View style={[styles.screen, isDark && { backgroundColor: '#000' }]}>
      ...
      <Text style={[styles.hint, { color: muted }]}>
        stretch розтягує вкладені елементи по висоті арени (якщо height не зафіксовано — тут height є, тож
...
![gap між children](/images/react-native/flexbox-and-layout/04.svg){.diagram-img}
...
![Shell header body footer](/images/react-native/flexbox-and-layout/03.svg){.diagram-img}
