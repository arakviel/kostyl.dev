...
Але варто відкрити майже будь-який зрілий застосунок на телефоні — і картина складніша. На вкладці «Стрічка» тап по картці відкриває **деталі**, при цьому **нижня панель вкладок часто лишається** (або зникає — залежно від продукту). Форма «щось швидко додати» виїжджає **зверху / як sheet**, а не «вклеюється» в ту саму історію, що й глибокий перегляд. Якщо користувач набрав текст і випадково свайпнув «назад», з’являється питання: **«Вийти без збереження?»** Окремо існує «світ гостя» (вхід / реєстрація) і «світ увійшовшого» (вкладки, профіль) — це вже не один плоский список екранів.
...
Перехід і вигляд modal ближчі до повноцінного екрана з material-анімацією (залежить від версії RN / Navigation). Системна кнопка **Back** зазвичай **поп**ає modal зі root Stack — тобто «закрити». Завжди проганяйте сценарій: відкрити форму з tabs → Back → чи повернулись на вкладку, а не вийшли з застосунку.
::
...
Екрани «Увійти» / «Зареєструватись» **не** повинні сидіти всередині `(tabs)` як «ще одна вкладка». І навпаки: стрічку поїздок **не** показують гостю так само, як увійшовшому (або показують, але з іншим guard — залежить від продукту).
...
## Nomad: trip details stack and modals
