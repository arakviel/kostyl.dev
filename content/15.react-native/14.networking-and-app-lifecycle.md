...
- **src/**
  - **api/**
    - **client.ts** — налаштований екземпляр axios / fetch з таймаутом 8 секунд
    - **types.ts** — TypeScript-інтерфейси курсів валют
  - **hooks/**
    - **useNetworkStatus.ts** — хук для відстеження `isInternetReachable`
    - **useAppState.ts** — хук для відстеження `AppState` (`active`/`background`)
    - **useRefetchOnFocusAndNetwork.ts** — хук для автоматичного поновлення
  - **components/**
    - **OfflineBanner.tsx** — плаваючий плашка-індикатор офлайну
    - **ErrorView.tsx** — екран помилки з кнопкою повтору
  - **screens/**
    - **CurrencyRatesScreen.tsx** — головний екран зі списком валют
- **App.tsx** — точка входу
...
::collapsible{title="Повний код додатка (App.tsx) — розгорнути для перегляду"}
...
- **db.json** — seed для `json-server` (масив `trips`)
- **app/**
  - **_layout.tsx** — `OfflineBanner` + `TripsProvider` + root Stack
  - **(tabs)/**
    - **trips/**
      - **index.tsx** — список: loading / error / pull-to-refresh / дані з API
- **src/**
  - **api/**
    - **client.ts** — axios (`baseURL` + timeout 8 с)
    - **tripsApi.ts** — `fetchTrips()`, `fetchTripById()`
    - **AppError.ts** — таксономія помилок → `userMessage`
  - **components/**
    - **OfflineBanner.tsx** — плашка «немає інтернету»
  - **hooks/**
    - **useNetworkStatus.ts** — `NetInfo` (`isOffline`)
    - **useAppState.ts** — `AppState`
    - **useRefetchOnFocusAndNetwork.ts** — refetch при active / reconnect
  - **features/trips/model/**
    - **TripsProvider.tsx** — завантаження з API, `refetch`, локальний `addTrip`
