...
1. **Закон Фіттса (Fitts's Law):** час, необхідний для точного позиціонування пальця на елементі керування, є функцією відстані до цілі та її геометричного розміру ($T = a + b \log_2(1 + D/W)$). Інтерфейсні елементи на сенсорному екрані повинні мати мінімальну фізичну площу детекції дотику (**Touch Target Size**) не менше 44×44 pt на iOS та 48×48 dp на Android.
...
**`Slider`** (повзунок) — елемент керування для вибору скалярного числового значення в замкненому інтервалі $[V_{\min}, V_{\max}]$ за допомогою неперервного або квантованого жестового перетягування.
...
Поточне числове значення повзунка. Має задовольняти нерівність $\text{minimumValue} \le \text{value} \le \text{maximumValue}$. Сам компонент не відображає текстовий лейбл значення, тому його виведення реалізується через суміжний елемент `<Text>`.
::
...
1. **Шар стану та валідації (Data Layer):** оперує чистими екземплярами класу `Date` або стандартизованими ISO-8601 рядками. Усі порівняння часових інтервалів ($\text{startDate} \le \text{endDate}$) виконуються за таймстемпами.
...
Поєднання бібліотек **React Hook Form** та **Zod** усуває необхідність створення десятків розрізнених `useState` та ручних перевірок умов.
...
```
    ...
    handleSubmit()  (React Hook Form)
           |
           v
    zodResolver(schema)  (@hookform/resolvers/zod)
           |
           +---------------------------------+
           |                                 |
       [ VALID ]                        [ INVALID ]
...
(Чисті типізовані дані)         (Inline повідомлення під полями)
```

### 1. Механізм оптимізації рендерингу React Hook Form
Традиційні форми на базі `useState` ініціюють повний ре-рендер батьківського дерева компонентів на кожен введений символ. React Hook Form використовує модель підписок (*Subscription-based proxy updates*): стан форми ізольований всередині рефа, а оновлення графіки відбуваються лише там, де змінюється значення або виникає помилка валідації.

### 2. Архітектурний міст `Controller`
Оскільки в React Native немає прямого доступу до DOM-рефів, компонент `Controller` виконує роль адаптера:
...
const tripSchema = z.object({
  title: z.string().trim().min(2, 'Назва має містити щонайменше 2 символи'),
  isPrivate: z.boolean(),
});
...
export function CreateTrip() {
  const {
    ...
    defaultValues: { title: '', isPrivate: false },
    mode: 'onSubmit',
    reValidateMode: 'onChange',
  });

  const onValid = (data: TripFormValues) => {
    console.log('Провалідовані дані:', data);
  };
      ...
      <Controller
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput
            ...
            onBlur={onBlur}
            placeholder="Подорож до Карпат"
          />
        )}
      />
      {errors.title && <Text style={{ color: 'red' }}>{errors.title.message}</Text>}

      <Pressable onPress={handleSubmit(onValid)} disabled={isSubmitting}>
        <Text>Зберегти</Text>
      </Pressable>
    </View>
  );
...
Адаптер декларативної валідації. Автоматично викликає `schema.safeParse()` під час сабміту та транслює згенеровані помилки Zod у формат `formState.errors`.
::

::field{name="defaultValues" type="DefaultValues<T>"}
Обов'язковий об'єкт початкових значень для всіх зареєстрованих полів форми. Запобігає неконтрольованому переходу зі стану `undefined` до значення, що викликає попередження React.
::

::field{name="mode" type="'onSubmit' | 'onChange' | 'onBlur' | 'onTouched' | 'all'"}
Визначає момент первинного запуску валідації форми. Стандартом для мобільних пристроїв є `mode: 'onSubmit'`, який не дратує користувача завчасними повідомленнями про помилки.
::

::field{name="reValidateMode" type="'onChange' | 'onBlur' | 'onSubmit'"}
Визначає алгоритм повторної валідації після першої невдалої спроби Submit. Режим `reValidateMode: 'onChange'` забезпечує миттєве зникнення повідомлення про помилку, щойно користувач виправив значення поля.
::

::field{name="watch" type="(name?: string) => any"}
Хук підписки на зміну значень окремих полів форми в реальному часі без ініціації сабміту (наприклад, для передачі дати початку `startDate` як мінімальної межі `minimumDate` у поле дати завершення).
::
