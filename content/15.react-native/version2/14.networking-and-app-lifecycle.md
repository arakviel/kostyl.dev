...
description: Академічне дослідження мережевої підсистеми React Native (fetch, axios, нативні стеки NSURLSession та OkHttpClient), нормалізація серверного стану, таксономія мережевих помилок, експоненційний backoff з jitter, черги оновлення токенів, моніторинг з'єднання через NetInfo та керування життєвим циклом застосунку через AppState.
---

# Робота з мережею та життєвий цикл застосунку

## Вступ: Мережева парадигма та життєвий цикл у мобільних операційних системах
...
У мобільній розробці на **React Native** середовище є **стохастичним, ресурсно обмеженим та високодинамічним**. Смартфон постійно переміщується у фізичному просторі, перемикається між стільниковими вежами різних стандартів (2G, 3G, LTE, 5G) та публічними точками доступу Wi-Fi. Під час цього процесу змінюються IP-адреси, виникають зони радіотіні (ліфти, підземні паркінги, тунелі) та спостерігається суттєва втрата мережевих пакетів (_packet loss_).

Одночасно з мережевою нестабільністю мобільна операційна система (iOS чи Android) веде жорсткий моніторинг енергоспоживання та використання оперативної пам'яті (RAM). Застосунок може бути щомиті переведений у фоновий режим, «заморожений» (_suspended_) або примусово завершений системним диспетчером (_Out-Of-Memory Killer_) без виклику деструкторів JavaScript-середовища.

### Навчальні цілі розділу:

- Дослідити фундаментальні відмінності між веб- та мобільною мережевою архітектурою: відсутність браузерного пісочника CORS, пряма взаємодія з нативними сокетами ОС.
- Зрозуміти шлях проходження мережевого запиту: від виклику в рушії **Hermes** через **JSI / TurboModules** до системних стеків **`NSURLSession`** (iOS) та **`OkHttpClient`** (Android).
- Проаналізувати роботу радіомодуля базової смуги (_Cellular Baseband Radio_) та концепцію станів керування радіоресурсами (**RRC States**), що впливають на автономність пристрою.
- Опанувати архітектуру **нормалізованого клієнтського стану (Normalized State: `byId` + `allIds`)**: чому збереження масивів об'єктів з API є антипатерном для мобільних додатків і як досягти складності вибірки та оновлення $O(1)$.
...
- Спроєктувати промисловий механізм **черги оновлення токенів (Mutex Token Refresh Queue)** на базі Axios Interceptors для усунення стану гонитви (_Race Condition_) при паралельних запитах з простроченим токеном.
- Опанувати наукову **таксономію помилок мережі (Error Taxonomy)**: розмежування транспортних збоїв, серверних відмов, бізнес-помилок авторизації та валідації схем (Zod).
- Засвоїти математичний апарат алгоритму **Exponential Backoff із рандомізованим Jitter** для запобігання системному феномену «шторму запитів» (_Thundering Herd Problem_).
- Спроєктувати типізований нормалізатор помилок **`AppError`** на базі дискримінованих об'єднань та enum-типів.
...
- Реалізувати патерн розумної ресинхронізації (**Smart Refetch on Focus & Reconnect**) для забезпечення консистентності стану мобільного інтерфейсу.
...
::note
**Фундаментальний постулат мобільної інженерії.** Мобільний клієнт повинен проєктуватися з розрахунку на те, що мережі **немає за замовчуванням**, будь-який запит може завершитися за таймаутом або обірватися на середині передачі байтів, а процес програми може бути знищений операційною системою будь-якої секунди після переходу у фоновий стан.
::

---

## Архітектурний місток: Від браузерного середовища до нативного мережевого стека
...
## Фізика мобільних мереж: Радіомодуль, стани RRC та витрата акумулятора
...
Радіомодуль стільникового зв'язку (LTE/5G) не може постійно працювати на повній потужності, оскільки акумулятор смартфона розрядився б за 2–3 години. Для економії енергії операційна система та модемний чип використовують кінцевий автомат станів **RRC**:
...
У веб-проєктах розробники часто зберігають такий масив безпосередньо в `useState<Trip[]>([])`. Проте в мобільних застосунках із насиченими списками, деталізацією, модальними вікнами та оптимістичними оновленнями збереження масиву стає **джерелом архітектурної деградації продуктивності**.

### Чому масиви об'єктів — це антипатерн для мобільного сховища

Розглянемо алгоритмічну складність типових операцій над масивом $N$ елементів:

1. **Пошук елемента за ідентифікатором (`find`):** Складність **$O(N)$**. При відкритті екрана деталей поїздки `/trips/t2` доводиться виконувати лінійний перебір усього масиву.
2. **Оновлення одиничного елемента (наприклад, лайк або перейменування):** Складність **$O(N)$**. Необхідно створити новий масив через `items.map(item => item.id === id ? { ...item, ...patch } : item)`.
3. **Видалення сутності зі списку:** Складність **$O(N)$** через `items.filter(item => item.id !== id)`.
...
| :--- | :--- | :--- |
| **Вибірка за ID (`getById`)** | $O(N)$ — `list.find()` | **$O(1)$** — `state.byId[id]` |
| **Оновлення сутності (`patch`)** | $O(N)$ — `list.map()` | **$O(1)$** — `state.byId[id] = { ...item, ...patch }` |
| **Видалення сутності (`delete`)** | $O(N)$ — `list.filter()` | **$O(1)$** у словнику + $O(N)$ у масиві ID |
| **Перевірка наявності (`has`)** | $O(N)$ — `list.some()` | **$O(1)$** — `Boolean(state.byId[id])` |
...
## Анатомія HTTP-клієнтів: `fetch` проти `axios`

У React Native стандартною є наявність глобальної функції `fetch`, що відповідає стандарту W3C/WHATWG Fetch API. Проте промислова розробка часто віддає перевагу спеціалізованим клієнтам, таким як `axios`. Розглянемо фундаментальну різницю між ними.

### Реалізація `fetch` у React Native та підводні камені

Поліфіл `fetch` у React Native побудований поверх внутрішньої нативної реалізації `XMLHttpRequest` (`RCTNetworking`). Незважаючи на стандартизований інтерфейс, пряме використання `fetch` у мобільних додатках несе суттєві архітектурні ризики:

::field-group

::field{name="1. Відсутність таймауту за замовчуванням" type="Архітектурний ризик"}
...
::field{name="2. Помилкова семантика HTTP-статусів" type="Архітектурний ризик"}
...
::field{name="3. Ручна двокрокова десеріалізація" type="Архітектурний ризик"}
...
### Чому індустріальним стандартом залишається `axios`

Бібліотека `axios` інкапсулює критичні для мобільної розробки функції в єдиний декларативний API:
...
## Проєктування архітектури мережевого шару на базі `axios`
...
### Промисловий патерн: Черга оновлення токенів (Refresh Token Mutex Queue)
...
## Таксономія помилок мережі та стійкість до відмов (Fault Tolerance)
...
### Математичний апарат: Exponential Backoff із рандомізованим Jitter
...
Інтервал очікування перед $n$-ю спробою розраховується за формулою:
...
- $t_{\text{base}}$ — базовий інтервал затримки (наприклад, 500 мс);
- $n$ — порядковий номер поточної спроби повтору ($n \in [0, N-1]$);
- $t_{\text{max}}$ — граничний ліміт інтервалу затримки (наприклад, 10 000 мс);
- $\text{random}(0, x)$ — функція рівномірного розподілу випадкової величини на відрізку $[0, x]$.
...
## Типізований нормалізатор помилок: `AppError`

Створимо надійну абстракцію `AppError`, що уніфікує гетерогенні винятки `axios`, стандартного `fetch` та рантайму JavaScript у строгий типізований формат:
...
### Дихотомія прапорців: `isConnected` проти `isInternetReachable`
...
## Життєвий цикл мобільного застосунку: Керування станами `AppState`
...
### Кінцевий автомат станів `AppState`
...
### Інженерні обов'язки при зміні `AppState`
...
## Синергія підсистем: Патерн Smart Refetch on Focus & Network
...
- Нормалізація отриманих даних (`byId` + `allIds`) для оптимізації $O(1)$;
...
### 3. Нормалізований `TripsProvider` зі швидким доступом $O(1)$
...
Збереження масивів з API сповільнює додаток до $O(N)$. Нормалізуйте дані в структуру `byId` + `allIds` для забезпечення вибірки та оновлення зі складністю $O(1)$.
::
...
::card{title="📊 Таксономія та Jitter" icon="i-lucide-shield-alert"}
...
При збереженні звичайного масиву `items.find()` та `items.map()` виконуються зі складністю $O(N)$ при кожному перегляді або лайку. У нормалізованому стані `{ byId, allIds }` будь-яка сутність вилучається та оновлюється миттєво за $O(1)$, а зміни відображаються узгоджено на всіх екранах.
::
