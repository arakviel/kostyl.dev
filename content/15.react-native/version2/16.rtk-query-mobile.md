...
Спочатку створюємо слайс (_slice_) для зберігання поїздок:
...
export function TripsListScreen() {
    ...
    return (
        <FlatList data={trips} keyExtractor={(item) => item.id} renderItem={({ item }) => <Text>{item.title}</Text>} />
    )
}
```
...
export const createTrip = createAsyncThunk('trips/createTrip', async (newTrip: Omit<Trip, 'id'>) => {
    const response = await fetch('http://localhost:3000/trips', {
        method: 'POST',
...
**Недолік:** Якщо інший користувач теж додав поїздку через веб-версію, ми про це не дізнаємось. Дані застаріють (_stale data_).
...
**RTK Query** (_RTK Query_) — це офіційне розширення Redux Toolkit, яке повністю автоматизує роботу з серверними даними. Ось як виглядає той самий функціонал з RTK Query:
...
::note
**Головний висновок:** RTK Query розуміє, що серверний стан (_Server State_) принципово відрізняється від клієнтського (_Client State_).
...
**Клієнтський стан** (_Client State_) — це дані, які **повністю належать вашому застосунку** та існують тільки на пристрої користувача. Ці дані завжди актуальні, бо їх джерело знаходиться у самому застосунку.
...
function CreateTripForm() {
    ...
    return (
        <View>
            <TextInput value={title} onChangeText={setTitle} placeholder="Назва поїздки" />
            {/* ... інші поля */}
        </View>
    )
}
...
**Серверний стан** (_Server State_) — це дані, **джерело істини яких знаходиться на віддаленому сервері**. Ваш застосунок зберігає лише тимчасову копію (кеш) цих даних.
...
Middleware (_проміжний обробник_) виконує важливі фонові завдання:
...
export function PostsScreen() {
    ...
    if (error) {
        return (
            <View style={styles.center}>
                <Text style={styles.errorText}>Помилка: {error.toString()}</Text>
            </View>
        )
    }
...
Теги (_tags_) — це ключова концепція RTK Query для автоматичної інвалідації кешу.
...
Найпростіший випадок — у вас є токен (_token_), який потрібно відправляти з кожним запитом у заголовку `Authorization`.
...
- Викликати _race condition_ (перший токен перезаписується другим)
- Призвести до інвалідації refresh token деякими backend системами

### Рішення: Mutex (взаємне блокування)

**Mutex** (_Mutual Exclusion_) — патерн, який гарантує, що тільки один запит може виконувати refresh у конкретний момент часу.
...
function TripsListScreen() {
    ...
    return (
        <View>
            ...
            <TextInput placeholder="Пошук..." value={search} onChangeText={setSearch} />

            <FlatList data={trips} renderItem={({ item }) => <TripCard trip={item} />} />
        </View>
    )
}
```
...
transformResponse: (response: Trip[]) => {
    ...
    return response.sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime())
}
```
...
return <FlatList data={data} refreshControl={<RefreshControl refreshing={isFetching} onRefresh={refetch} />} />
```
...
function SearchScreen() {
    ...
    return (
        <View>
            <TextInput value={tripId} onChangeText={setTripId} placeholder="Введіть ID поїздки" />
...
Тепер, коли ми навчилися **читати** дані через Query, час навчитися їх **змінювати**. Mutations (_мутації_) — це операції, які модифікують дані на сервері: створення, оновлення, видалення.
...
Уявіть, що ви натискаєте "Створити поїздку" двічі — має створитись **дві** різні поїздки, а не повернутись результат першого запиту. Мутації мають **побічні ефекти** (_side effects_) — вони змінюють стан сервера.
...
export function CreateTripScreen() {
    ...
    return (
        <View style={{ padding: 20 }}>
            <TextInput placeholder="Назва поїздки" value={title} onChangeText={setTitle} style={styles.input} />

            <TextInput placeholder="Регіон" value={region} onChangeText={setRegion} style={styles.input} />
            ...
            {error && <Text style={styles.error}>Помилка: {error.toString()}</Text>}
        </View>
    )
}
```
...
function TripCard({ trip }) {
    ...
    const handleDelete = () => {
        Alert.alert('Підтвердження', `Видалити поїздку "${trip.title}"?`, [
    ...
    return (
        <View style={styles.card}>
            <Text>{trip.title}</Text>
            <Button title={isLoading ? '...' : '🗑️'} onPress={handleDelete} disabled={isLoading} />
        </View>
    )
}
```
...
export const tripsApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getTrips: builder.query<Trip[], void>({
            ...
            providesTags: (result) =>
                result
                    ? [...result.map((trip) => ({ type: 'Trip' as const, id: trip.id })), { type: 'Trip', id: 'LIST' }]
                    : [{ type: 'Trip', id: 'LIST' }],
        }),
...
providesTags: (result) =>
    result
        ? [...result.map((trip) => ({ type: 'Trip', id: trip.id })), { type: 'Trip', id: 'LIST' }]
        : [{ type: 'Trip', id: 'LIST' }]
```
...
export const api = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getTrips: builder.query<Trip[], void>({
            query: () => '/trips',
            providesTags: (result) =>
                result ? [...result.map((trip) => ({ type: 'Trip' as const, id: trip.id })), 'Trip'] : ['Trip'],
        }),

        // Коментарі до поїздки
        getTripComments: builder.query<Comment[], string>({
            query: (tripId) => `/trips/${tripId}/comments`,
            providesTags: (_result, _error, tripId) => [{ type: 'Comment', id: `TRIP-${tripId}` }],
        }),
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.formRow}>
                <TextInput style={styles.input} placeholder="Нова нотатка..." value={title} onChangeText={setTitle} />
    ...
    return (
        <View style={styles.card}>
            ...
            <TouchableOpacity onPress={handleFavoritePress}>
                <Text style={styles.icon}>{trip.isFavorite ? '❤️' : '🤍'}</Text>
            </TouchableOpacity>
        </View>
    )
}
...
function InfiniteTripsScreen() {
    ...
    return (
        <FlatList
            ...
            ListFooterComponent={isFetching ? <ActivityIndicator style={{ padding: 20 }} /> : null}
        />
    )
}
```
...
export const liveTrackingApi = createApi({
    endpoints: (builder) => ({
        getLiveLocation: builder.query<{ lat: number; lng: number }, string>({
            ...
            async onCacheEntryAdded(driverId, { updateCachedData, cacheDataLoaded, cacheEntryRemoved }) {
                // Створюємо WebSocket з'єднання
