...
#output
| Апаратний пристрій | Розмір матриці | Час множення ($N \times N$) | Прискорення (Speedup) |
| :--- | :---: | :---: | :---: |
| **Центральний процесор (CPU)** | $5000 \times 5000$ | `8.234` с | $1.0\times$ (базовий рівень) |
| **Графічний процесор (GPU CUDA)** | $5000 \times 5000$ | **`0.095` с** | 🚀 **$86.7\times$ швидше** |

![Порівняння продуктивності GPU/CPU та оптимізація пам'яті за допомогою Automatic Mixed Precision](/images/ai-python/pytorch-intro/advanced-topics/01.png){.diagram-img}
...
#output
| Параметр відеокарти | Значення | Статус |
| :--- | :--- | :---: |
| **CUDA доступна** | `True` | ✅ Активовано |
| **Кількість GPU** | `1` | Одиночний прискорювач |
| **Поточний індекс** | `0` (`cuda:0`) | За замовчуванням |
| **Модель GPU** | `NVIDIA GeForce RTX 3080` | High-End |
| **Загальна пам'ять VRAM** | `10.00 GB` | Готово для батчів |
...
#output
| Об'єкт | Значення тензора | Пристрій (`device`) | Тип оперативної пам'яті |
| :--- | :--- | :---: | :--- |
| `t_cpu` | `tensor([1., 2., 3.])` | `cpu` | Системна пам'ять RAM |
| `t_gpu` | `tensor([1., 2., 3.], device='cuda:0')` | `cuda:0` | Відеопам'ять GPU VRAM |
...
#output
| Стан моделі | Пристрій параметрів | Пам'ять | Вплив на обчислення |
| :--- | :---: | :--- | :--- |
| **Після створення (`model`)** | `cpu` | RAM | Обчислення на ядрах CPU |
| **Після `model.to(device)`** | `cuda:0` | GPU VRAM | ⚡ Паралельне виконання на ядрах CUDA |
...
#output
| Епоха | Значення втрат (Loss) | Пристрій тренування | Статус навчання |
| :---: | :---: | :---: | :--- |
| **1 / 10** | `0.5234` | `cuda:0` | Початкова оптимізація |
| **3 / 10** | `0.3412` | `cuda:0` | Стабільне сходження |
| **5 / 10** | `0.2145` | `cuda:0` | Мінімізація похибки |
| **8 / 10** | `0.1345` | `cuda:0` | Тонке донавчання |
| **10 / 10** | **`0.0987`** | `cuda:0` | 🎯 Навчання завершено успішно |
...
#output
| Характеристика апаратного прискорювача | Значення | Статус підтримки AMP |
| :--- | :---: | :---: |
| **Модель GPU** | `NVIDIA GeForce RTX 3080` | ✅ Підтримується |
| **Compute Capability** | `8.6` (Ampere Architecture) | ✅ Повна апаратна підтримка |
| **Tensor Cores** | Наявні 3-го покоління | 🚀 Апаратне прискорення FP16/BF16 |
...
#output
| Режим точності | Значення функції втрат | Різниця втрат ($\Delta$) | Висновок щодо точності |
| :--- | :---: | :---: | :--- |
| **Стандартний FP32** | `2.3456` | — | Базова повна точність |
| **Mixed Precision (AMP FP16)** | `2.3452` | `0.0004` ($0.017\%$) | ✅ **Ідентична якість без деградації** |
...
#output
| Епоха | Loss (FP16 AMP) | Поточний масштаб GradScaler | Швидкість тренування | Економія VRAM |
| :---: | :---: | :---: | :---: | :---: |
| **1 / 10** | `1.8234` | `65536.0` | **$2.1\times$ швидше** | **$-49\%$** пам'яті |
| **3 / 10** | `1.2145` | `65536.0` | **$2.1\times$ швидше** | **$-49\%$** пам'яті |
| **5 / 10** | `0.8456` | `65536.0` | **$2.1\times$ швидше** | **$-49\%$** пам'яті |
| **8 / 10** | `0.5234` | `65536.0` | **$2.1\times$ швидше** | **$-49\%$** пам'яті |
| **10 / 10** | **`0.3412`** | `65536.0` | 🏆 **$2.1\times$ швидше** | 🏆 **$-49\%$** пам'яті |
...
#output
| Операція в циклі | Час виконання | Частка від загального часу |
| :--- | :---: | :---: |
| **Forward pass** | `0.234 ms` | $42.5\%$ |
| **Matrix multiplication** | `0.156 ms` | $28.4\%$ |
| **ReLU activation** | `0.045 ms` | $8.2\%$ |
| **Backward pass** | `0.312 ms` | $56.7\%$ |
...
#output
| Назва операції (`Name`) | CPU Time (%) | CPU Total | CUDA Time (%) | CUDA Total | Кількість викликів |
| :--- | :---: | :---: | :---: | :---: | :---: |
| `aten::mm` (Matrix Multiply) | $32.4\%$ | `1.245 ms` | $58.2\%$ | `0.450 ms` | 10 |
| `aten::add` (Vector Add) | $12.1\%$ | `0.465 ms` | $14.3\%$ | `0.110 ms` | 10 |
| `aten::relu` (Activation) | $8.5\%$ | `0.326 ms` | $9.8\%$ | `0.076 ms` | 10 |
| `aten::linear` (Full Layer) | $47.0\%$ | `1.805 ms` | $17.7\%$ | `0.137 ms` | 5 |
...
#output
| Конфігурація DataLoader | Параметри | Час епохи (с) | Прискорення | Завантаження GPU |
| :--- | :--- | :---: | :---: | :---: |
| **Базова (CPU дебаг)** | `num_workers=0, pin_memory=False` | `2.845` с | $1.0\times$ | $45\%$ (GPU простоює) |
| **З асинхронним завантаженням** | `num_workers=4, pin_memory=False` | `1.120` с | $2.5\times$ | $78\%$ |
| **Оптимальна (Production)** | `num_workers=4, pin_memory=True` | **`0.845` с** | 🚀 **$3.4\times$ швидше** | **$96\%$** (максимальна утилізація) |
...
#output
| Шар архітектури | Ключ у `state_dict` | Розмірність тензора | Тип даних |
| :--- | :--- | :---: | :---: |
| **Вхідний шар** | `'fc1.weight'` | `torch.Size([64, 10])` | `torch.float32` |
| | `'fc1.bias'` | `torch.Size([64])` | `torch.float32` |
| **Вихідний шар** | `'fc2.weight'` | `torch.Size([1, 64])` | `torch.float32` |
...
#output
| Етап операції | Дія | Результат верифікації |
| :--- | :--- | :--- |
| **Збереження ваг** | `torch.save(model.state_dict(), 'model_weights.pth')` | ✅ Файл ваг збережено на диску |
| **Створення нової моделі** | `new_model = SimpleModel()` | Ініціалізовано випадковими вагами |
| **Завантаження ваг** | `new_model.load_state_dict(...)` | ✅ Ваги відновлено |
| **Звірка виходів** | `torch.allclose(out_orig, out_loaded)` | 🎯 **100% ідентичні виходи (Zero error)** |
...
#output
| Параметр чекпоінта | Збережене значення | Стан після відновлення |
| :--- | :---: | :--- |
| **Номер епохи (`epoch`)** | `10` | Тренування відновлено з епохи `11` |
| **Валідаційні втрати (`loss`)** | `0.2345` | Використовується для Early Stopping |
| **Стан оптимізатора (`optimizer_state`)** | Біжучі моменти Adam збережено | Оптимізація продовжується без ривків |
| **Стан моделі (`model_state`)** | Усі ваги та bias | 🎯 Повна відтворюваність |
...
#output
| Режим моделі | Час інференсу | Потреба у Python Runtime | Сумісність з C++ / Production |
| :--- | :---: | :---: | :---: |
| **PyTorch (Eager Mode)** | `0.450 ms` | Обов'язковий (Python) | Тільки в Python-середовищі |
| **TorchScript (Traced JIT)** | **`0.280 ms`** | ❌ Не потрібен | 🚀 Прямий запуск у C++, Java, iOS, Android |
| **Збіг виходів** | $\Delta < 10^{-7}$ | ✅ Повна чисельна еквівалентність | — |
...
#output
| Тестовий вхід $X$ | Умова в моделі ($X.\text{sum}() > 0$) | Результат PyTorch | Результат Scripted | Збіг |
| :---: | :---: | :---: | :---: | :---: |
| `[ 1., 2., 3.]` ($>0$) | Гілка `x * 2` | `[2., 4., 6.]` | `[2., 4., 6.]` | ✅ Збігається |
| `[-1., -2., -3.]` ($\le 0$) | Гілка `x * 0` | `[0., 0., 0.]` | `[0., 0., 0.]` | ✅ Збігається |
...
#output
| Середовище виконання | Формат моделі | Максимальна чисельна розбіжність ($\Delta_{\max}$) | Статус інтеграції |
| :--- | :--- | :---: | :--- |
| **PyTorch (`torch.nn.Module`)** | Python / PyTorch runtime | — | Еталонне значення |
| **ONNX Runtime (`onnxruntime`)** | `model.onnx` Graph | **`0.0000000`** ($< 10^{-7}$) | 🏆 **Готово до кросплатформного деплою** |
