...
| **Пам'ять VRAM** | Достатньо для батчів $128 \times 3 \times 32 \times 32$ | ✅ Оптимізовано |
...
#output
| Характеристика датасету CIFAR-10 | Значення | Опис і семантика |
| :--- | :---: | :--- |
| **Загальний обсяг** | **`60,000`** кольорових зображень | 50,000 навчальних + 10,000 тестових |
| **Роздільна здатність** | **`32 × 32 × 3`** | 3 канали (RGB), 3072 числа на зображення |
| **Кількість класів** | **`10`** класів по 6,000 зображень | Збалансовані реальні об'єкти та тварини |
| **Перелік класів** | `['літак', 'автомобіль', 'птах', 'кіт', 'олень', 'собака', 'жаба', 'кінь', 'корабель', 'вантажівка']` | 6 видів тварин + 4 види транспортних засобів |
...
#output
| Блок архітектури | Тип шару | Конфігурація | Розмірність тензора | Кількість параметрів |
| :--- | :--- | :--- | :---: | :---: |
| **Вхідний шар** | `Input` | 3 канали RGB | `(128, 3, 32, 32)` | $0$ |
| **Згортковий блок 1** | `Conv2d` + `BatchNorm2d` + `ReLU` | 64 фільтри $3 \times 3$, pad 1 | `(128, 64, 32, 32)` | $1,856$ |
| **Пулінг 1** | `MaxPool2d(2, 2)` | Зменшення розміру $2\times$ | `(128, 64, 16, 16)` | $0$ |
| **Згортковий блок 2** | `Conv2d` + `BatchNorm2d` + `ReLU` | 128 фільтрів $3 \times 3$, pad 1 | `(128, 128, 16, 16)` | $74,112$ |
| **Пулінг 2** | `MaxPool2d(2, 2)` | Зменшення розміру $2\times$ | `(128, 128, 8, 8)` | $0$ |
| **Згортковий блок 3** | `Conv2d` + `BatchNorm2d` + `ReLU` | 256 фільтрів $3 \times 3$, pad 1 | `(128, 256, 8, 8)` | $295,680$ |
| **Пулінг 3** | `MaxPool2d(2, 2)` | Зменшення розміру $2\times$ | `(128, 256, 4, 4)` | $0$ |
| **Векторизація** | `Flatten` | $256 \times 4 \times 4$ | `(128, 4096)` | $0$ |
| **Повнозв'язний шар 1**| `Linear(4096, 512)` + `ReLU` | Прихований шар | `(128, 512)` | $2,097,664$ |
| **Регуляризація** | `Dropout(p=0.5)` | Випадкове занулення | `(128, 512)` | $0$ |
| **Класифікатор** | `Linear(512, 10)` | 10 класів виходу | `(128, 10)` | $5,130$ |
| **Загальний обсяг** | — | — | — | **`2,897,738` параметрів** |
...
#output
| Розподіл вибірки | Зразків | Розмір батчу | Кількість батчів |
| :--- | :---: | :---: | :---: |
| **Train DataLoader** | 50,000 | 128 | **`391`** |
| **Test DataLoader** | 10,000 | 128 | **`79`** |
...
#output
::alert{type="success"}
✅ **Тренувальний конвеєр налаштовано:** Підготовлено спільну функцію навчання з шедулером `CosineAnnealingLR` для чесного порівняння двох підходів: базової моделі (без аугментації) та регуляризованої моделі (з RandomHorizontalFlip, RandomCrop, ColorJitter).
::
...
#output
| Епоха | Без аугментації (Train Loss / Val Acc) | З аугментацією (Train Loss / Val Acc) | Поточний LR | Динаміка перенавчання |
| :---: | :---: | :---: | :---: | :--- |
| **5 / 30** | `0.8421` / 68.45% | `1.2145` / 63.21% | `0.000951` | Базова мережа швидше запам'ятовує вибірку |
| **10 / 30** | `0.5234` / 72.18% | `0.9823` / 70.45% | `0.000809` | Початок стагнації валідації без аугментації |
| **15 / 30** | `0.3124` / 73.84% | `0.8234` / 75.67% | `0.000588` | Аугментована модель випереджає базову |
| **20 / 30** | `0.1867` / 74.12% | `0.7012` / 78.92% | `0.000309` | Без аугментації: Val Loss зростає до 1.08 |
| **25 / 30** | `0.1021` / 73.98% | `0.6123` / 80.34% | `0.000095` | Явний оверфітинг базової (Train Acc 96.3%) |
| **30 / 30** | **`0.0612` / 74.05%** | **`0.5534` / `81.23%`** | `0.000003` | 🏆 **+7.18% Val Accuracy завдяки аугментації** |

> ⏱️ **Час тренування:** ~10-12 хвилин на GPU для кожної моделі.
...
| Метрика ефективності | Модель без аугментації | Модель з Data Augmentation | Ефект регулярзації |
| :--- | :---: | :---: | :--- |
| **Train Accuracy (фінальна)** | **`97.89%`** | `80.89%` | Запобігає запам'ятовуванню конкретних пікселів |
| **Val Accuracy (фінальна)** | `74.05%` | **`81.23%`** | 🏆 **+7.18%** покращення реальної узагальнюючої здатності |
| **Найкраща Val Accuracy** | `74.12%` | **`81.23%`** | Стабільна збіжність без просідання на пізніх епохах |
| **Overfitting Gap (Train − Val)** | **`23.84%`** ⚠️ | **`-0.34%`** ✅ | Повна ліквідація розриву перенавчання |
...
| Клас об'єкта | Точність (Precision) | Повнота (Recall) | $F_1$-міра | Кількість у тесті | Ключові сплутування |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **`літак`** | `0.8423` | `0.8610` | `0.8515` | 1000 | Частково з птахами та кораблями (спільне небо/вода) |
| **`автомобіль`** | `0.9012` | `0.9230` | `0.9120` | 1000 | Висока точність, поодинокі сплутування з вантажівками |
| **`птах`** | `0.7234` | `0.7010` | `0.7120` | 1000 | Сплутування з літаками та котами |
| **`кіт`** | `0.6523` | `0.6380` | `0.6451` | 1000 | ⚠️ Найскладніший клас: 142 сплутування з собаками |
| **`олень`** | `0.7812` | `0.7920` | `0.7866` | 1000 | Поодинокі сплутування з кіньми та собаками |
| **`собака`** | `0.7145` | `0.7490` | `0.7314` | 1000 | 126 сплутувань з котами (схожі морди й шерсть) |
| **`жаба`** | `0.8534` | `0.8810` | `0.8670` | 1000 | Висока повнота завдяки унікальному зеленому кольору |
| **`кінь`** | `0.8723` | `0.8560` | `0.8641` | 1000 | Поодинокі сплутування з оленями |
| **`корабель`** | `0.8912` | `0.8750` | `0.8830` | 1000 | Висока точність завдяки водному фону |
| **`вантажівка`** | `0.8945` | `0.8630` | `0.8785` | 1000 | 58 сплутувань з легковими авто |
| **У середньому (Total):** | **`0.8126`** | **`0.8123`** | **`0.8131`** | **10,000** | 🎯 **Загальна точність (Accuracy): 81.23%** |
