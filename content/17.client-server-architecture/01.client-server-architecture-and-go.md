...
import (
	...
	"github.com/student/my-server/internal/config"
	"github.com/student/my-server/internal/handler"
	"github.com/student/my-server/pkg/logger"
)
...
func main() {
	...
	// 4. Запуск сервера
	addr := ":" + cfg.Port
	logger.Info("Запуск HTTP сервера на порту " + addr)
...
import (
	...
	"github.com/student/my-server/internal/service"
)
...
import "github.com/student/my-server/internal/repository"
...
**Як працюють імпорти між пакетами?**

У прикладі вище зверніть увагу на структуру імпортів:
...
::field{name="go mod download" type="Завантаження залежностей"}
...
::field{name="go mod verify" type="Перевірка цілісності"}
...
::field{name="go clean -modcache" type="Очищення кешу"}
...
::field{name="go list -m all" type="Перегляд залежностей"}
...
**Ключова відмінність Go від інших:**

У Go **одна утиліта `go` замінює 10+ окремих інструментів** з інших екосистем:
