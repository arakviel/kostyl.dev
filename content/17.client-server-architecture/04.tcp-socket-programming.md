...
description: "Сокетне програмування мовою Go: сокети Берклі (Berkeley Sockets API) як файлові дескриптори, спостереження TCP-сегментів через tcpdump і netcat, життєвий цикл TCP-сесії (3-Way Handshake, 4-Way Close, стан TIME_WAIT), примітиви net.Dial і net.Listen, архітектура «горутина на з'єднання» з runtime netpoller, потокове введення/виведення з io та bufio, три методи фреймінгу прикладних протоколів (розділювачі, префікс довжини, TLV), транслятор повідомлень на базі Hub (CSP) та плавне завершення (Graceful Shutdown)."
---
...
Саме тому мережеві абстракції мови Go поєднуються з моделлю конкурентності: операційна система сигналізує про нове TCP-з'єднання, середовище виконання делегує його обробку окремій горутині, а життєвий цикл координується через канали та контексти. Для використання мережевого API необхідно розпочати з базового рівня: зрозуміти, як операційна система ініціалізує сокети та контролює рух байтових сегментів мережею.
...
Сторона, яка першою ініціювала закриття з'єднання, переходить у системний стан **`TIME_WAIT`** і утримує зайняту пару портів протягом часу :math-formula{tex="2 \\times \\text{MSL}" inline} (*Maximum Segment Lifetime* — максимальний час існування сегмента в мережі, зазвичай 30–120 секунд):

::math-formula
T_{\text{WAIT}} = 2 \times \text{MSL}
::
...
## Реалізація клієнта та сервера

Для освоєння сокетів створимо програми клієнта та сервера, які можна скопіювати в окремі файли та запустити у двох терміналах.

### Реалізація TCP-клієнта
...
### Проблема послідовного сервера: блокування головного циклу

Перш ніж перейти до масштабованої архітектури, розглянемо наївну послідовну реалізацію сервера. Що станеться, якщо функція обслуговування клієнта `handleConnection(conn)` викликається **синхронно** всередині головного циклу прийняття з'єднань?

Нижче наведено робочий приклад такого синхронного сервера:

```go [cmd/sequential_server/main.go]
package main
...
func main() {
	...
	log.Println("Синхронний сервер слухає :9000 (лише 1 клієнт одночасно)...")
	...
	for {
		...
		// Синхронний виклик заморожує головний цикл! Сервер не зможе
		// викликати наступний Accept(), доки поточний клієнт не закриє з'єднання.
	...
	for {
		...
		if err != nil {
			if !errors.Is(err, io.EOF) {
				log.Printf("Помилка читання від %s: %v", remoteAddr, err)
			}
			log.Printf("[-] Клієнт %s відключився", remoteAddr)
			return
		}

		log.Printf("Отримано від %s: %s", remoteAddr, string(buffer[:n]))
		if _, err := conn.Write(buffer[:n]); err != nil {
			log.Printf("Помилка запису: %v", err)
			return
		}
...
Чому синхронна модель не підходить для мережевого сервера:
1. **Блокування черги:** Головна горутина зупиняється у виклику `handleConnection(conn)` на невизначений час. Якщо клієнт підключився і мовчить, сервер не прийме нових з'єднань.
2. **Черга очікування ОС (*backlog*):** Якщо під час роботи з першим клієнтом підключаються інші користувачі, їхнє потрійне рукостискання завершується на рівні ядра ОС, але самі вони залишаються заблокованими в черзі *backlog*, оскільки програма не викликає `Accept()`. Коли черга переповнюється, нові клієнти отримують помилку підключення.
3. **Неефективне використання ресурсів:** Процесорні ядра простоюють, обслуговуючи одне повільне введення/виведення замість одночасної обробки запитів.

::warning
Синхронна обробка допустима виключно в одноразових CLI-утилітах або простих скриптах тестування. Для реального сервера така модель не підходить, оскільки один повільний клієнт блокує роботу всього сервісу.
::

### Конкурентний відлуння-сервер (Goroutine-Per-Connection)

Розв'язання проблеми послідовного сервера в Go полягає в тому, що завдяки легкості горутин достатньо додати ключове слово **`go`** перед викликом `handleConnection(conn)`.

Головна горутина циклу `Accept()` миттєво делегує нове підключення окремій горутині й повертається до очікування наступних клієнтів. Кожен клієнт отримує власну незалежну горутину з власним стеком і локальними змінними:
...
У Go обмеження пулу одночасних підключень реалізується через патерн **лічильного семафора на буферизованому каналі**. Буфер каналу ємністю :math-formula{tex="N" inline} задає кількість доступних слотів для підключень. Захоплення слота здійснюється через запис у канал `sem <- struct{}{}`, а звільнення — через читання `<-sem`.

Нижче наведено сервер із захистом від перевантаження: якщо ліміт вичерпано, сервер негайно інформує клієнта та закриває з'єднання, не витрачаючи пам'ять на нову горутину-обробник:
...
Перед корисним навантаженням надсилається фіксована кількість байтів (зазвичай 2 або 4 байти), які кодують ціле число — довжину тіла повідомлення. Отримувач спочатку читає фіксовану кількість байтів заголовка, дізнається довжину :math-formula{tex="L" inline}, а потім вичитує рівно :math-formula{tex="L" inline} байтів тіла. Для гарантованого вичитування повної порції байтів стандартна бібліотека надає функцію `io.ReadFull`.
...
### Демонстраційна програма кадрування TLV

Наведений нижче лістинг демонструє запис повідомлень у єдиний байтовий потік та їх відновлення.
...
Традиційний підхід вимагав би зберігання списку клієнтів у структурі `map[net.Conn]bool`, захищеній м'ютексом `sync.Mutex`. Проте підхід у Go базується на моделі CSP (*Communicating Sequential Processes*): **«Не спілкуйтеся через спільну пам'ять; натомість діліться пам'яттю, спілкуючись через канали»**.

Патерн **Hub** централізує керування станом у єдиній виділеній горутині, запобігаючи стану гонитви (*race condition*).

### Сервер багатокористувацького чату

Нижче наведено код чат-сервера. Його можна запустити та підключитися з кількох терміналів за допомогою `nc localhost 9000`.
...
### Реалізація сервера із Graceful Shutdown

Нижче наведено код сервера, який перехоплює сигнали операційної системи `SIGINT` (Ctrl+C) та `SIGTERM`.
...
## Практичне завдання: сервер із контролем активності (Heartbeat)
