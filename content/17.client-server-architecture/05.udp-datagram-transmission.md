...
description: "Розгляд протоколу UDP мовою Go: спостереження сирих датаграм без рукостискання через nc -u та tcpdump, заголовок UDP (RFC 768), обмеження MTU та загроза IP-фрагментації, примітиви net.ListenPacket, net.DialUDP та net.PacketConn, налаштування буферів сокета ОС, режими адресації Unicast, Broadcast і Multicast з протоколом IGMP, архітектура пулу воркерів (Worker Pool) для збору телеметрії та концепція RUDP (Reliable UDP)."
---
...
- Сформувати модель датаграми (*datagram*) як самостійної, атомарної одиниці передачі з чіткими межами повідомлення.
- Дослідити 8-байтний заголовок UDP (RFC 768) та розрахувати безпечний розмір корисного навантаження з урахуванням MTU (*Maximum Transmission Unit*).
...
**Протокол UDP (*User Datagram Protocol*, RFC 768)** є альтернативою: він свідомо відмовляється від усіх перелічених гарантій. UDP не встановлює з'єднання, не відстежує доставку, не сортує пакети та не контролює перевантаження мережі.
...
## Структура заголовка UDP та характеристики протоколу
...
3. **Відсутність збереження черговості (*no ordering guarantee*):** пакети маршрутизуються незалежно; датаграма :math-formula{tex="N+1" inline} може дійти швидше за датуграму :math-formula{tex="N" inline}.
...
Теоретично поле довжини в 16 бітів дозволяє відправити датаграму розміром до 65 535 байтів (з урахуванням 8 байтів заголовка максимальне навантаження для IPv4 становить 65 507 байтів). Проте спроба надіслати датаграму такого розміру через реальну мережу призведе до різкого падіння надійності.
...
- Якщо ймовірність втрати одного фрагмента становить :math-formula{tex="p" inline}, то ймовірність успішної доставки датаграми з :math-formula{tex="k" inline} фрагментів становить :math-formula{tex="(1-p)^k" inline}.
...
::math-formula
\text{Payload}_{\text{safe}} = \text{MTU} - \text{IP}_{\text{header}} - \text{UDP}_{\text{header}} = 1500 - 20 - 8 = 1472
::

Отже, безпечний розмір корисного навантаження для стандартного Ethernet становить 1472 байти.
...
### UDP-сервер та клієнт

Нижче наведено програми, які можна скопіювати в окремі файли та запустити одночасно.
...
Багатьох інженерів дивує наявність слова `Dial` («додзвонитися» / встановити з'єднання) стосовно протоколу UDP, адже UDP принципово беззв'язний (*connectionless*).

Коли програма викликає `net.Dial("udp", ...)` або `net.DialUDP(...)`:
1. **Жодних мережевих пакетів не надсилається** — тристороннє рукостискання (3-way handshake) відсутнє.
2. На рівні ядра операційної системи виконується системний виклик `connect()`.
3. Сокет стає «прив'язаним» (*connected UDP socket*) до вказаної адреси в пам'яті ядра хоста.

Завдяки прив'язці на рівні ядра:
- Можна використовувати звичайні потокові методи `conn.Write(buf)` та `conn.Read(buf)` замість передачі адрес у `WriteTo` / `ReadFrom` (ОС автоматично підставляє адресу призначення і знає, від кого очікувати відповідь).
- Ядро ОС автоматично фільтрує та відкидає пакети від будь-яких сторонніх IP-адрес, крім зафіксованої.
- Ядро здатне обробляти зворотні повідомлення `ICMP Port Unreachable` (код відсутності слухача на віддаленому порту), повертаючи помилку `connection refused` при наступному читанні або записі (звичайний незв'язаний UDP-сокет такі ICMP-повідомлення мовчки ігнорує).
...
##### `net.Dial` проти `net.DialUDP`: чому саме `DialUDP`?

Використовувати загальний виклик `net.Dial("udp", "127.0.0.1:9001")` **можна**, і це цілком валідний код у Go. Проте між `net.Dial` та спеціалізованим `net.DialUDP` існують дві суттєві відмінності: **контроль локальної адреси/порту** та **тип, що повертається**.

###### 1. Контроль локальної адреси та порту (`laddr`)

Сигнатури функцій у стандартній бібліотеці Go:

```go
// Приймає лише мережу та віддалену адресу
func Dial(network, address string) (Conn, error)

// Дозволяє явно зафіксувати локальну адресу (laddr)
func DialUDP(network string, laddr, raddr *UDPAddr) (*UDPConn, error)
```

Коли клієнт відправляє UDP-пакет, операційна система за замовчуванням виділяє йому випадковий вільний локальний порт (*ephemeral port*).

- Якщо передати `laddr = nil` у `DialUDP`, поведінка буде точно такою ж, як у `Dial` (ОС обере порт автоматично).
- Але якщо інженерному завданню **критично виходити з конкретного локального порту або IP-інтерфейсу** (наприклад, у протоколах VoIP/SIP, потоковому медіа RTP, DNS або сценаріях обходу NAT через STUN/TURN), у `net.Dial` цього зробити неможливо — тут потрібен саме `net.DialUDP`:

```go
localAddr, _ := net.ResolveUDPAddr("udp", "0.0.0.0:40000") // фіксований локальний порт клієнта
serverAddr, _ := net.ResolveUDPAddr("udp", "198.51.100.1:9001")

conn, err := net.DialUDP("udp", localAddr, serverAddr)
```

###### 2. Тип, що повертається: загальний `net.Conn` проти конкретного `*net.UDPConn`

- `net.Dial` повертає абстрактний інтерфейс **`net.Conn`**. Через нього доступні лише базові методи: `Read()`, `Write()`, `Close()`, `LocalAddr()`, `RemoteAddr()` та `SetDeadline()`.
- `net.DialUDP` повертає конкретну структуру **`*net.UDPConn`**. Вона дає прямий доступ до низькорівневих можливостей сокета UDP на рівні ядра ОС:
  - `ReadMsgUDP` / `WriteMsgUDP` — читання/запис позасмугових даних (*out-of-band / OOB*), таких як значення IP TTL, розширені заголовки IPv4/IPv6 або апаратні часові мітки.
  - Налаштування системних буферів сокета ОС (`SetReadBuffer`, `SetWriteBuffer`) без необхідності вдаватися до приведення типів `conn.(*net.UDPConn)`.
  - Керування параметрами мультикасту та широкомовлення (TTL групової розсилки, прапори loopback тощо).

###### Підсумок: коли що обирати

| Сценарій | `net.Dial("udp", "host:port")` | `net.DialUDP("udp", laddr, raddr)` |
| :--- | :--- | :--- |
| **Повернений тип** | `net.Conn` (інтерфейс) | `*net.UDPConn` (конкретна структура) |
| **Фіксація локального порту (`laddr`)** | ❌ Неможливо (тільки випадковий ephemeral port) | ✅ Повний контроль через параметр `laddr` |
| **Низькорівневий тюнінг сокета** | Потребує приведення типів `.(*net.UDPConn)` | ✅ Прямий доступ (`SetReadBuffer`, OOB control) |
| **Коли обирати** | Прості клієнти «запит-відповідь» (DNS, метрики StatsD), де влаштовує загальний контракт | VoIP/SIP, медіа-стрімінг, STUN/NAT traversal, сервіси з вимогами до локального порту або буферів ядра |
...
## Сервер телеметрії з пулом воркерів

При розробці навантажених серверів UDP не слід створювати нову горутину `go handle(packet)` на кожну вхідну датаграму, оскільки за великого потоку пакетів це призводить до швидкого вичерпання пам'яті (*unbounded concurrency*).

На практиці використовують **фіксований пул воркерів (*Worker Pool*)**, які забирають завдання через буферизований канал, та агрегацію результатів у виділеній горутині (патерн CSP).

Нижче наведено два файли: сервер телеметрії та клієнт-генератор.
...
## Практичне завдання: утиліта UDP Ping
...
Системний виклик `connect()` для сокета UDP не надсилає жодних пакетів у мережу (handshake відсутній). Він лише фіксує в пам'яті ядра ОС адресу та порт сервера для цього сокета. Це активує фільтрацію стороннього трафіку ядром, дозволяє викликати спрощені методи `Read`/`Write` замість `ReadFrom`/`WriteTo`, а також дозволяє операційній системі перехоплювати пакети `ICMP Port Unreachable` і транслювати їх у помилку `connection refused`.

:::

:::accordion-item{label="❓ Чому варто використовувати net.DialUDP замість net.Dial(\"udp\", ...)?" icon="i-lucide-help-circle"}

Обидва виклики створюють connected UDP-сокет, проте `net.DialUDP` надає дві ключові переваги:
1. Дозволяє передати `laddr` і жорстко зафіксувати локальний порт або інтерфейс клієнта (необхідно для VoIP, STUN/NAT traversal, DNS), тоді як `net.Dial` завжди виділяє випадковий порт.
2. Повертає конкретний тип `*net.UDPConn` замість загального інтерфейсу `net.Conn`, що відкриває доступ до низькорівневих можливостей ядра: налаштування буферів `SetReadBuffer`/`SetWriteBuffer`, позасмугових даних OOB через `ReadMsgUDP`/`WriteMsgUDP` та опцій мультикасту.
