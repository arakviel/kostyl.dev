...
JavaScript є **інтерпретованою мовою**: браузери та середовище Node.js спочатку створювалися для виконання JavaScript-коду безпосередньо, рядок за рядком, без попереднього перетворення. TypeScript, навпаки, класичні рушії JavaScript (V8 у Chrome/Node.js, SpiderMonkey у Firefox, JavaScriptCore у Safari) не розуміють нативно, оскільки синтаксис анотацій типів є невалідним для стандарту ECMAScript.

::note
**Чи підтримує сучасний Node.js виконання TypeScript напряму?**

Починаючи з версії **Node.js 22.6.0** (із подальшим розширенням у версіях 22.7+ та 23+), у середовище додано експериментальну підтримку запуску TypeScript-файлів без сторонніх транспіляторів безпосередньо через механізм **видалення типів** (_type stripping_): `node --experimental-strip-types app.ts` (або просто `node app.ts` у новіших релізах).

Проте слід чітко розрізняти підхід Node.js від компілятора `tsc`:
- **Без перевірки типів (_no type checking_):** Node.js на льоту лише вирізає синтаксис типів (за допомогою вбудованого парсера `amaro` на базі `SWC`) і одразу передає JavaScript рушію V8. Якщо у коді є логічні помилки типізації, Node.js їх проігнорує і виконає код аж до першої runtime-помилки.
- **Обмеження синтаксису:** Механізм видалення типів підтримує лише анотації, що стираються без сліду. Він не підтримує синтаксичні розширення TypeScript, які генерують власний JavaScript-код під час виконання (переліки `enum`, простори імен `namespace` або `parameter properties` у конструкторах), якщо не увімкнено додаткові прапорці трансформації.

Отже, навіть за наявності швидкого запуску в сучасному Node.js (чи середовищах на кшталт Deno та Bun), компілятор `tsc` залишається обов'язковим інструментом для **статичного аналізу і гарантії безпеки типів** на етапі перевірки та збірки проєкту (`tsc --noEmit`).
...
rectangle TSC #FCD34D [
<b>TypeScript Compiler</b>
(tsc)
--
1. Парсинг коду
2. Перевірка типів
...
TS -right-> TSC : npx / pnpm tsc example.ts
...
::terminal-preview{title="tsc --version" :cursor="false"}
...
Команди `npx` (для npm) та `pnpm` запускають локально встановлений виконуваний файл з каталогу `node_modules/.bin/`, що дозволяє уникнути глобального встановлення компілятора в операційну систему.
...
```bash
pnpm tsc math.ts --target ES2020
```
...
::tip
Команда `npx tsc --help` (або `pnpm tsc --help`) виводить повний список доступних параметрів компілятора. Експериментуйте з різними комбінаціями для розуміння їхнього впливу на вихідний код.
::
...
```bash
pnpm tsc src/index.ts --target ES2020 --module ESNext --outDir dist --strict --esModuleInterop
```
...
::note
Якщо у каталозі є `tsconfig.json`, команда `tsc` (через `npx tsc` або `pnpm tsc`) без аргументів компілює **весь проєкт** згідно з конфігурацією. Якщо вказати конкретний файл (`npx tsc file.ts` або `pnpm tsc file.ts`), `tsconfig.json` **ігнорується**, і використовуються параметри за замовчуванням або параметри CLI.
::
...
Параметр `strict: true` вмикає всі суворі перевірки типів одночасно. Це **найважливіший параметр** для якісного коду.
...
При зміні файлу доводиться перекомпілювати весь проєкт командою `npx tsc --watch` (або `pnpm tsc --watch`) та вручну перезавантажувати браузер. Немає автоматичного оновлення змін у реальному часі.
...
## Еволюція архітектури: Vite 8+ та єдиний рушій Rolldown

У попередніх підрозділах ми розглянули класичну архітектуру Vite (версії 2–7), яка базувалася на поєднанні двох різних інструментів: `esbuild` для швидкої трансформації модулів у процесі розробки та `Rollup` для фінальної збірки у продакшен. Проте з виходом **Vite 8+** в екосистемі відбулася масштабна інженерна трансформація — перехід на **Rolldown**, єдиний надшвидкий бандлер нового покоління, написаний на мові Rust.

### Проблема «подвійного рушія» у класичному Vite (Vite 2–7)

Класична архітектура Vite досягала рекордної швидкості за рахунок компромісного інженерного розділення:

1. **Під час розробки (_Development_):** використовувався **esbuild** (написаний на Go). Він миттєво здійснював транспіляцію синтаксису TypeScript у JavaScript та попередньо збирав npm-залежності у формат ES-модулів.
2. **Під час продакшен-збірки (_Production Build_):** викликався **Rollup** (написаний на JavaScript). Саме він виконував глибинний аналіз графа залежностей, оптимізацію, усунення мертвого коду (_tree-shaking_), розбиття на чанки (_code splitting_) та застосування складних плагінів.

Хоча такий підхід зробив революцію у швидкості локальної розробки, він містив у собі концептуальну ваду — **розбіжність поведінки між середовищами** (_Dev/Prod Inconsistency_):
- **Різні рушії трансформації:** те, що успішно компілювалося й працювало локально через esbuild, іноді призводило до неочікуваних збоїв чи іншої семантики коду після збірки бандлером Rollup.
- **Складність екосистеми плагінів:** авторам плагінів доводилося або дублювати логіку для esbuild та Rollup, або писати складні абстракції-адаптери.
- **Вузьке місце продуктивності на великих масштабах:** хоча локальний запуск був миттєвим, продакшен-збірка величезних комерційних проєктів на JavaScript-рушії Rollup залишалася ресурсомісткою та повільною.

### Архітектурний стрибок у Vite 8+: єдине Rust-ядро Rolldown

У версії **Vite 8+** команда розробників (проєкт **VoidZero** під керівництвом Евана Ю) кардинально переробила фундамент збирача: обидва інструменти (`esbuild` та `Rollup`) було замінено **єдиним уніфікованим бандлером Rolldown**.

::plant-uml{alt="Порівняння архітектури: Класичний Vite проти Vite 8+"}
...
package "Класичний Vite (Vite 2–7)" #F8FAFC {
    rectangle "Розробка (Dev)\nesbuild (Go)" as DevClassic #FEF3C7
    rectangle "Продакшен (Build)\nRollup (JS)" as ProdClassic #FEE2E2
    DevClassic -[hidden]down-> ProdClassic
    note right of DevClassic
      Два різні рушії:
      - Розбіжності Dev/Prod
      - Складні адаптери плагінів
      - Повільніший фінальний білд
    end note
}

package "Сучасний Vite (Vite 8+)" #F0FDF4 {
    rectangle "Єдине Rust-ядро: Rolldown" as Unified #DCFCE7 {
        rectangle "Dev HMR та парсинг на льоту" as Dev8 #BBF7D0
        rectangle "Prod бандлінг і Tree-shaking" as Prod8 #86EFAC
    }
    note right of Unified
      Єдиний Rust-конвеєр:
      - 10-30x прискорення збірки
      - 100% паритет Dev / Prod
      - Повна сумісність з Rollup API
    end note
}
@enduml
```
...
### Ключові нововведення та переваги Vite 8+

1. **Повна узгодженість розробки та збірки (_Dev/Prod Parity_):**
   Один і той самий парсер (на базі високопродуктивного набору інструментів **Oxc**) та єдиний алгоритм побудови графа модулів застосовуються як для локального dev-сервера, так і для команди `vite build`. Розробник отримує стовідсоткову впевненість: те, що протестовано локально, працюватиме у продакшені абсолютно ідентично.

2. **Нативна продуктивність Rust (прискорення у 10–30 разів):**
   Завдяки відмові від виконання бандлінгу всередині віртуальної машини Node.js/V8 та відсутності пауз збирача сміття (_Garbage Collection_), час продакшен-збірки середніх та великих TypeScript-проєктів скорочується в десятки разів.

3. **Сумісність з API плагінів Rollup та Vite:**
   Незважаючи на написання рушія з нуля на Rust, Rolldown відтворює інтерфейс викликів Rollup Plugin API. Це означає, що переважна більшість існуючих плагінів Vite продовжує працювати без змін у конфігураціях `vite.config.ts`.

4. **Місце TypeScript у конвеєрі Vite 8+:**
   Незважаючи на заміну внутрішнього рушія на Rolldown, архітектурний розподіл відповідальності для мови TypeScript залишається незмінним:
   - Rolldown блискавично виконує видалення анотацій типів (_Type Stripping_) безпосередньо у машинному коді Rust.
   - Компілятор TypeScript (`tsc`) продовжує відповідати виключно за **статичну валідацію типів** через параметр `"noEmit": true` у `tsconfig.json` або команду `tsc --noEmit` у пайплайнах неперервної інтеграції (CI/CD).
...
- **Vite 8+ та Rolldown** — еволюційний стрибок до єдиного нативного Rust-ядра (Rolldown), що усуває розбіжності між dev/prod та прискорює збірку у 10–30 разів.
...
1. Чому TypeScript не може виконуватися класичним Node.js або браузером без компіляції, і в чому полягає суть Type Stripping у сучасному Node.js?
2. Яка різниця між компіляцією через `npx tsc file.ts` (чи `pnpm tsc file.ts`) та запуском `tsc` без аргументів?
...
9. У чому полягала проблема «подвійного рушія» у класичному Vite (esbuild + Rollup) та яку перевагу дає єдине Rust-ядро Rolldown у Vite 8+?
