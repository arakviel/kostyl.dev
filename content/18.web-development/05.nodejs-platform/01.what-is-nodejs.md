...
rectangle JSCode #DBEAFE [
JavaScript код
]
rectangle Parser #E2E8F0 [
Parser
]
rectangle AST #FEF3C7 [
AST
Abstract Syntax Tree
]
rectangle Ignition #DBEAFE [
Ignition
Байт-код інтерпретатор
]
rectangle Hot #FEF3C7 [
Гарячий код?
]
rectangle TurboFan #DCFCE7 [
TurboFan
Оптимізуючий компілятор
]
rectangle Execute #E2E8F0 [
Виконання
]
...
const server = http.createServer(async (req, res) => {
  try {
    const content: string = await fs.readFile('./data.json', 'utf8');
...
1. **Ініціалізація:** V8 компілює JavaScript-код.
...
5. **Обробка запиту:** при надходженні HTTP-запиту спрацьовує callback `(req: IncomingMessage, res: ServerResponse)`.
6. **Читання файлу:** `await fs.readFile()` передає операцію у пул потоків, а `await` звільняє Event Loop.
7. **Повернення результату:** після завершення читання `try`-блок продовжується з прочитаним вмістом.
...
http.createServer((req, res) => {
  const result: number = fibonacci(45); // Обчислення займає ~10 секунд
...
function runWorker(n: number): Promise<number> {
  return new Promise((resolve, reject) => {
    const worker = new Worker('./fibonacci-worker.js', {
      workerData: { n } satisfies { n: number }
    });
...
http.createServer(async (req, res) => {
  const result: number = await runWorker(45);
  res.end(`Fibonacci(45) = ${result}`);
}).listen(3000);
```
...
const child: ChildProcess = fork('./compute-service.js');
...
Node.js традиційно використовував систему CommonJS, але сучасні проєкти переходять на ES Modules:
...
::terminal-preview{title="tsx src/server.ts"}
