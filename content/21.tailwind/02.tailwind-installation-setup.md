...
## Метод 2: Tailwind CLI (статичні сайти)

**Tailwind CLI** — автономний інструмент збірки без Vite або webpack. Його сильна сторона — простота налаштування. Ідеально підходить для:

- **Статичних сайтів** (HTML + CSS без JavaScript-фреймворку)
- **Прототипування** без повноцінної системи збірки
- **Jekyll, Hugo, Eleventy** — генератори статичних сайтів
- **Навчання** Tailwind у чистому вигляді, без toolchain

### Встановлення CLI

```bash
# Ініціалізація Node-проєкту
npm init -y
...
│   └── index.html      ← використовує Tailwind-класи
├── dist/
│   └── output.css      ← генерується автоматично (git-ігнор)
└── package.json
```
...
# Мінімізована production-збірка
...
::caution
CDN-версія Tailwind — це **не для продакшну**. Без build step не буде оптимізації: завантажується весь Tailwind (~112 КБ), а не тільки потрібні класи. Кастомні токени через `@theme` також не підтримуються в CDN-режимі.
::
...
<html lang="uk">
    ...
    <body class="bg-gray-100 p-8">
        <div class="max-w-md mx-auto bg-white rounded-xl shadow p-6">
            ...
            <p class="text-slate-500 mt-2">Швидкий старт без збірки</p>
        </div>
    </body>
</html>
```
...
| Метод | Підходить для | Нода | Плюси | Мінуси |
|---|---|---|---|---|
| **CDN** | Прототипи, демо | Не потрібен | Міттєво почати, без інсталяцій | ~112 КБ CSS, без `@theme` |
| **CLI** | Статичні сайти, Jekyll, Hugo | Node.js | Просто, без bundler, `@theme` працює | Без HMR |
| **Vite-плагін** | Vite, React, Vue, Svelte | Node.js + Vite | Найшвидший HMR, просто налаштовую | Требує Vite |
| **PostCSS** | webpack, Rollup, Parcel | Node.js + bundler | Універсальне, будь-який bundler | Требує `postcss.config.js` |
| **Next.js** | Next.js застосунки | Node.js + Next.js | Вбудована інтеграція | Обмежено Next.js екосистемою |
| **Nuxt 3** | Nuxt застосунки | Node.js + Nuxt | Вбудована інтеграція | Обмежено Nuxt |

---

## Налаштування редактора

Правильно налаштований редактор — різниця між задоволенням та болем від Tailwind. Витратьте 5 хвилин зараз, зекономте години потім.
...
## Як Tailwind v4 знаходить класи: `@source`

У Tailwind v3 треба було вказувати `content: ['./src/**/*.html']` у конфігурації. У v4 — ні, Tailwind автоматично сканує всі `.html`, `.js`, `.ts`, `.jsx`, `.tsx`, `.vue`, `.svelte` файли поряд з вхідним CSS.

Якщо необхідно вказати додаткові шляхи (нестандартні розширення):
...
/* Виключити legacy-папку */
@source not "../legacy/**";
```

**Safelist: класи, що генеруються динамічно**

Якщо ваш JS створює класи динамічно — Tailwind не побачить їх при скануванні:

```js
// ❌ Так не працює: Tailwind не бачить результуючий рядок при збірці
const color = getUserColor() // 'red', 'blue'...
...
/* Додати конкретні класи до збірки */
@source inline("bg-red-500 bg-blue-500 bg-green-500");

/* Або через шаблон (ЕФЕКТИВНІШЕ) */
