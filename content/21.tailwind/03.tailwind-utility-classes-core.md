...
### Повна таблиця скорочень

| Скорочення | CSS-властивість | Приклад |
|---|---|---|
| `p` | `padding` (всі сторони) | `p-4` → `padding: 1rem` |
| `px` | `padding-left + padding-right` | `px-6` → горизонтальний |
| `py` | `padding-top + padding-bottom` | `py-3` → вертикальний |
| `pt` | `padding-top` | `pt-2` → зверху |
| `pr` | `padding-right` | `pr-4` → праворуч |
| `pb` | `padding-bottom` | `pb-6` → знизу |
| `pl` | `padding-left` | `pl-3` → ліворуч |
| `ps` | `padding-inline-start` | `ps-4` → logical, RTL-ready |
| `pe` | `padding-inline-end` | `pe-4` → logical, RTL-ready |
| `m` | `margin` (всі сторони) | `m-4` → `margin: 1rem` |
| `mx` | `margin-left + margin-right` | `mx-auto` → горизонтально центр |
| `my` | `margin-top + margin-bottom` | `my-8` → вертикальний |
| `mt` | `margin-top` | `mt-4` → відступ зверху |
| `mr` | `margin-right` | `mr-2` → праворуч |
| `mb` | `margin-bottom` | `mb-6` → знизу |
| `ml` | `margin-left` | `ml-auto` → відштовхнути праворуч |
| `ms` | `margin-inline-start` | `ms-4` → logical |
| `me` | `margin-inline-end` | `me-4` → logical |
| `w` | `width` | `w-full`, `w-1/2`, `w-64` |
| `h` | `height` | `h-screen`, `h-48` |
| `size` | `width + height` (НОВА v4) | `size-8` → `w-8 h-8` |
| `min-w` | `min-width` | `min-w-0` → flex fix |
| `max-w` | `max-width` | `max-w-xl` → обмеження |
| `gap` | `gap` (flex/grid) | `gap-4` → `1rem` між елементами |
| `space-x` | `margin-left` між дочірніми | `space-x-4` |
| `space-y` | `margin-top` між дочірніми | `space-y-2` |

::tip
Скорочення `s/e` (start/end) — логічні властивості для RTL-підтримки. `ps-4` = `padding-left` для LTR, `padding-right` для RTL. Використовуйте їх замість `l/r` для міжнародних застосунків.
::
...
`size-*` — нова в Tailwind v4 утиліта, що встановлює `width` і `height` одночасно. Це не просто скорочення — це семантично вірний спосіб визначити **квадратний** або **рівносторонній** елемент:
...
```html
<!-- ❌ v3: потрібно два класи + ризик розсинхронізації -->
<div class="w-12 h-12 rounded-full bg-indigo-100">

<!-- ✅ v4: один клас, читабельно і безпечно -->
<div class="size-12 rounded-full bg-indigo-100">

<!-- Комбінуємо з варіантами так само як w-* h-* -->
<div class="size-8 hover:size-10 transition-all duration-200 rounded-full bg-indigo-500">
<!-- Анімований аватар що збільшується при hover -->
```

**Таблиця: `size-*` vs `w-* h-*`**

| Завдання | Стара форма | Нова форма (v4) |
|---|---|---|
| Аватар 48px | `w-12 h-12` | `size-12` |
| Іконка 20px | `w-5 h-5` | `size-5` |
| Кнопка-квадрат | `w-10 h-10` | `size-10` |
| Повна ширина/висота | `w-full h-full` | `size-full` |
| Hover: більший | `hover:w-12 hover:h-12` | `hover:size-12` |

`size-*` приймає ті самі значення, що `w-*`: числа шкали (`size-4`, `size-8`), дроби (`size-1/2`), спеціальні (`size-full`, `size-screen`, `size-auto`, `size-fit`).
