...
## `@custom-variant` у контексті теми

Директива `@custom-variant` — природне продовження `@theme`. Якщо `@theme` визначає **токени** (що виглядає), то `@custom-variant` визначає **умови** (коли застосовується). Разом вони утворюють повну систему адаптивного дизайну:
...
```html
<!-- Використання: токени + варіанти разом -->
<html class="dark">
    <div class="bg-white dark:bg-slate-900
                theme-ocean:bg-cyan-950
                p-4 compact:p-2
...
Найгнучкіший підхід для dynamic theming — комбінація CSS-змінних у `:root` та `@custom-variant`:

```css [src/styles/theme.css]
/* Крок 1: Примітивні токени (фіксовані значення) */
@theme {
...
/* Крок 2: Семантичні токени (змінюються в залежності від теми) */
:root {
...
/* Крок 3: Dark тема перевизначає семантичні токени */
.dark {
    --color-bg:      var(--color-brand-900);
    --color-fg:      var(--color-brand-50);
    --color-primary: oklch(0.65 0.2 277); /* світліший для темного фону */
}

/* Крок 4: Варіант для застосування в HTML */
@custom-variant dark (&:is(.dark *));
```
...
<div class="bg-(--color-bg) text-(--color-fg)">
    Вміст автоматично адаптується
</div>
```

::tip
`bg-(--color-bg)` — скорочений синтаксис v4 для `bg-[var(--color-bg)]`. Дужки навколо імені змінної вказують Tailwind на CSS-custom property.
::

---

## Архітектура шарів CSS: base → components → utilities

Tailwind організовує CSS у три шари з чіткою ієрархією. Розуміння цієї ієрархії критично важливо для правильного використання `@layer`:

::mermaid

```mermaid
graph TD
    A["@import 'tailwindcss'"] --> B["@layer base"]
    A --> C["@layer components"]
    A --> D["@layer utilities"]

    B --> B1["Reset стилі"]
    B --> B2["Глобальні body, h*, a"]
    B --> B3["@font-face"]
    B --> B4["Найнижча специфічність"]

    C --> C1[".btn, .card, .badge"]
    C --> C2["@apply з базових утиліт"]
    C --> C3["Семантичні класи"]
    C --> C4["Перевизначає base"]

    D --> D1["p-4, text-xl, flex"]
    D --> D2["@utility власні утиліти"]
    D --> D3["Найвища специфічність"]
    D --> D4["Завжди перемагають"]
...
@layer base {
    ...
    a { @apply text-brand-500 hover:text-brand-600 transition-colors; }
}

/* components — середній шар */
@layer components {
    .card {
        @apply bg-white rounded-xl border border-slate-200 shadow-sm p-6;
    }
    .btn {
        @apply inline-flex items-center justify-center gap-2
               px-5 py-2.5 font-semibold rounded-xl transition-colors text-sm;
    }
    .btn-primary {
        @apply bg-brand-500 text-white hover:bg-brand-600;
    }
...
/* utilities (через @utility) — найвищий шар */
@utility glass {
