...
Кастомні шрифти — через `@theme` кількома способами:

**Спосіб 1: Google Fonts через `<link>` у HTML**

```html [index.html]
<head>
    <!-- Підключення через link (рекомендований для Next.js / статичних сайтів) -->
...
**Спосіб 2: Google Fonts через `@import` у CSS**

```css [src/input.css]
/* УВАГА: @import має бути першим у файлі, до @import 'tailwindcss' */
...
**Спосіб 3: Локальні шрифти через `@font-face`**

```css [src/input.css]
@import 'tailwindcss';

/* Оголошення локального шрифту */
@font-face {
    font-family: 'BrandFont';
    src: url('/fonts/brand-font.woff2') format('woff2'),
         url('/fonts/brand-font.woff')  format('woff');
    font-weight: 400 900;  /* variable font — весь діапазон */
    font-style: normal;
    font-display: swap;    /* Важливо для продуктивності */
}
...
|---|---|---|---|
| `<link>` у HTML | Статичні сайти, Next.js | Швидко завантажується (preconnect) | Потрібна зміна HTML |
| `@import` у CSS | Прості проєкти | Все в CSS | Блокує рендер якщо перший |
| `@font-face` + local | Корпоративні шрифти | Повний контроль | Потрібно хостити шрифти |

```html
<h1 class="font-display font-black">Заголовок на display-шрифті</h1>
<p class="font-sans">Текст на sans-serif</p>
```
