...
- **Текст — основа статті, код — лише ілюстрація до тексту.** Стаття — це не зразок коду з коментарями; це розгорнуте текстове пояснення, де код підкріплює думку.
- **Розгорнуті пояснення**: Кожна концепція має супроводжуватися кількома абзацами тексту — контекст, аналогія з реального світу, пояснення «чому саме так» та «що буде, якщо зробити інакше».
- **Code Anatomy**: Кожен нетривіальний блок коду розбирається за схемою: контекст → код → декомпозиція (рядок за рядком або блок за блоком). Читач має розуміти не лише _що_ робить код, а _чому_ він написаний саме так.
- **Обсяг тексту**: Прагніть до того, щоб обсяг пояснювального тексту значно перевищував обсяг коду. Якщо у розділі 30 рядків коду — до нього має бути мінімум стільки ж рядків тексту з поясненнями.

### 4. **Show, Don't Just Tell**: Візуалізуйте все, що можна. Текст — це клей між візуальними елементами.

### 5. **Interactivity**: Використовуйте компоненти Docus, щоб зробити читання активним процесом.

### 6. Академічна Точність та Гнучкість
...
- **Live Preview**: `::html-preview` — інтерактивний HTML/CSS-пісочниця (кастомний компонент)

### Компонент `::html-preview` (Live HTML/CSS Preview)

Кастомний компонент для відображення **живого результату** HTML та CSS коду безпосередньо в статті. Він автоматично знаходить блоки коду всередині себе (`html`/`markup` та `css`), об'єднує їх у документ і рендерить в ізольованому `<iframe>`.

**Коли використовувати:** у статтях про HTML, CSS — скрізь, де читач має побачити візуальний результат коду.
...
- Блок `css` є **необов'язковим** — якщо він відсутній, застосовуються лише базові системні стилі.
- Висота **автоматично адаптується** під контент через `ResizeObserver` + `postMessage` — смуги прокрутки відсутні.
- Код виконується в ізольованому `sandbox` — зовнішні скрипти безпечні.
