...
Є загальний термін “AJAX” (абревіатура від **A**synchronous **J**avaScript **A**nd **X**ML) для мережевих запитів від JavaScript коду. Але формат XML використовувати не обов’язково: цей термін застарілий, тому це слово (XML) тут. Можливо, ви вже його десь чули.
...
-   **`response.formData()`** – повертає відповідь, як об’єкт `FormData` (він буде розглянутий [у наступному розділі](https://uk.javascript.info/formdata)),
-   **`response.blob()`** – повертає відповідь, як [Blob](https://uk.javascript.info/blob) (бінарні дані з типом),
-   **`response.arrayBuffer()`** – повертає відповідь, як [ArrayBuffer](https://uk.javascript.info/arraybuffer-binary-arrays) (низькорівневе представлення двійкових даних),
...
Для прикладу роботи із бінарними даними, буде зроблено запит та виведено на екран логотип [специфікації “fetch”](https://fetch.spec.whatwg.org/) (див. розділ [Blob](https://uk.javascript.info/blob), щоб дізнатись детальніше про операції із `Blob`):
...
## [Заголовки відповіді](https://uk.javascript.info/fetch#zagolovki-vidpovidi)
...
## [Заголовки запиту](https://uk.javascript.info/fetch#zagolovki-zapitu)
...
## [POST запити](https://uk.javascript.info/fetch#post-zapiti)
...
-   **`body`** – тіло запиту, щось одне із списку:
    ...
    -   [URLSearchParams](https://uk.javascript.info/url), для відправлення даних у кодуванні `x-www-form-urlencoded`, використовується рідко.
...
## [Відправлення зображення](https://uk.javascript.info/fetch#vidpravlennya-zobrazhennya)
...
## [Підсумки](https://uk.javascript.info/fetch#pidsumki)
...
-   **`response.blob()`** – повертає об’єкт як [Blob](https://uk.javascript.info/blob) (бінарні дані з типом),
-   **`response.arrayBuffer()`** – повертає відповідь як [ArrayBuffer](https://uk.javascript.info/arraybuffer-binary-arrays) (низько рівневі бінарні дані),
