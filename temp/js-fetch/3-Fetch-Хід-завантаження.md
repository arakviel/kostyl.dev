Метод `fetch` дозволяє відстежувати хід _завантаження_.

Будь ласка, зверніть увагу: наразі `fetch` не може відстежувати хід _вивантаження_. Для цієї мети використовуйте [XMLHttpRequest](https://uk.javascript.info/xmlhttprequest), ми розглянемо його пізніше.
...
2.  Перед зчитуванням ми можемо визначити повну довжину відповіді із заголовка `Content-Length`.
    ...
    Він може бути відсутнім для запитів між джерелами (дивись розділ [Fetch: Запити між різними джерелами](https://uk.javascript.info/fetch-crossorigin)), і, взагалі-то, технічно сервер не зобов’язаний його встановлювати. Але зазвичай він присутній.
...
5.  Маємо результат у `chunksAll`. Але це байтовий масив, а не рядок.
    ...
    Щоб створити рядок, нам потрібно інтерпретувати ці байти. Вбудований [TextDecoder](https://uk.javascript.info/text-decoder) робить саме це. Потім ми можемо перетворити рядок на дані за допомогою `JSON.parse`, якщо необхідно.
...
Ще раз зауважте, що це не для процесу _вивантаження_ даних на сервер (зараз немає змоги використовувати `fetch`) – лише для процесу _завантаження_ даних з сервера.
