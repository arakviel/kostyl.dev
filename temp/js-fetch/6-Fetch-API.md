...
Ми повністю висвітлили `method`, `headers` та `body` у главі [Fetch](https://uk.javascript.info/fetch).

Опція `signal` висвітлена у главі [Fetch: Переривання](https://uk.javascript.info/fetch-abort).

Тепер розгляньмо решту можливостей.

## [referrer, referrerPolicy](https://uk.javascript.info/fetch-api#referrer-referrerpolicy)
...
Політика встановлення Referrer (Referrer Policy) стосується не тільки `fetch`
...
## [mode](https://uk.javascript.info/fetch-api#mode)

Опція `mode` є засобом безпеки, що запобігає випадковим запитам між різними джерелами:

-   **`"cors"`** – запити між різними джерелами типово дозволені, як описано в [Fetch: Запити між різними джерелами](https://uk.javascript.info/fetch-crossorigin),
...
## [credentials](https://uk.javascript.info/fetch-api#credentials)
...
-   **`"include"`** – завжди надсилати, вимагає `Access-Control-Allow-Credentials` від сервера іншого джерела, щоб JavaScript міг отримати доступ до відповіді, про що йдеться у главі [Fetch: Запити між різними джерелами](https://uk.javascript.info/fetch-crossorigin),
-   **`"omit"`** – ніколи не надсилати, навіть для запитів з однаковим джерелом.

## [cache](https://uk.javascript.info/fetch-api#cache)
...
## [redirect](https://uk.javascript.info/fetch-api#redirect)
...
## [integrity](https://uk.javascript.info/fetch-api#integrity)
...
## [keepalive](https://uk.javascript.info/fetch-api#keepalive)
